 ...
 * handed to demangle.  It is returned if it isnotpossible to
 ...
 * else if (dm == mn)
 * 	printf("name couldnotbe demangled\n");
 * else
 ...
 * String.  For this reason, the pointer should nameOfThis
 ...
 * that isnotat the very beginning
...
int clen;
{
	...
	if(!eq__(c)) {
		/* If a name doesnotbegin with a __
		...
		/* Operator cast doesnotfit the mould
 ...
 *	the string.  It returns 0 if __pt__ isnotfound.
 */
