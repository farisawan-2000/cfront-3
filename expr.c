...
// error('d'," f().  %n mem->tp %t",e1->fct_name,mem->tp);
            ...
            switch (b->base) {
                default:
                    if (mem->base == DTOR) { // i.int::~int(), etc.
                                             ...
                                             //					error("T mismatch for simpleT destructor:XO
                                             //ofT%t",mem->tp2);
                        return mem;
                    } else
            ...
            */
            ...
            if (mem->tp) {
                ...
                for (Pexpr ee = mem; ee->base == REF; ee = ee->e1) {
                    if (ee->e1 == cc->c_this) { // this-> => p->
                        ee->e1 = e1;
                        ee->base = base;
                        if (bcc && mem->tp && mem->tp->skiptypedefs()->base != FCT
            ...
            for (Pexpr ee = e; ee->base == REF; ee = ee->e1) {
                if (ee->e1 == cc->c_this) { // this-> => p->
                    ee->e1 = e1;
                    ee->base = base;
                    if (bcc && e->tp && e->tp->skiptypedefs()->base != FCT
            ...
            switch (e1->base) {
                ...
                case REF:       // becomes ob.x::~x(0) or
                    if (e2 == 0 // becomes p->x::~x(0)
                        && Pref(e1)->mem && Pref(e1)->mem->base == DTOR) {
                        Pref r = Pref(e1);
                        if (r->base == DOT) {
    ...
    switch (b) { // filter out non-overloadable operators
        default: {
            ...
            if (x) {
                if (x->tp != any_type && (b == INCR || b == DECR) && x->base != INCR
                    && x->base != DECR) {
#if 0
				if (t2==0 && t1) { // postfix
					Pexpr ox = x;
    ...
    switch (b) { /* are the operands of legal types */
        case REFMUL:
            base = REF;
            // no break;
        case DOT: { // a .* p => &a MEMPTR p => appropriate indirection
            ...
            if (pcn) {
                ...
                for (Pname on = cl->conv; on; on = on->n_list) {
                    ...
                    if (frt->base != PTR || Pptr(frt)->memof == 0)
                        break;
        ...
        case DEREF:
            ...
            if (e2 == dummy)
                error("subscriptE missing");

            if (t) { /*	*t	*/
                t = t->skiptypedefs();
                if (t->base == PTR && Pptr(t)->memof)
                    error("P toM dereferenced");
                tp = t->deref();
                        ...
                        switch (t->base) {
                            ...
                            case FCT:
                                // error('d', "cn->tp: %d, %k", cn->tp, cn->tp->base);
                                {
                                    ...
                                    e1 = new expr(ELIST, zero, e1);
                                    // e2 is the name
                                    if (Pname(e2)->n_oper == ASSIGN
                                        && cl->memtbl->look(e2->string, 0)->tp->base != OVERLOAD) {
                                        Pname nmw = new name;
                                        *nmw = *((Pname) e2);
                            ...
                            default:
                                if (n2->n_stclass != STATIC) { // offset + 1
                                    ...
                                    // error('d',"cl %d %s i
                                    // %d",Pclass(cn->tp),Pclass(cn->tp)?Pclass(cn->tp)->string:"0",n2->n_offset);
                                } else
                                    return this;
                        }
                        // error('d',"int_type");
                    ...
                    case REF: {
                        ...
                        if (f->base == FCT || f->base == OVERLOAD) { // &p->f
                            ...
                            if (f->base == OVERLOAD && all_stat == 0
                                ...
                                error("address of boundF (try using ``%s::*'' forPT and ``&%s::%s'' "
                                      "for address)",
                                      m->n_table->t_name->string, m->n_table->t_name->string,
                                      m->string);
                            }
    ...
    if (e1 == dummy || e1 == 0)
        error("operand missing for%k", b);
    if (e2 == dummy || e2 == 0)
        error("initializer missing for%k", b);
    ...
    switch (b) {
        ...
        case NE:
            ...
            if (r1 == 'P' && t1->memptr() || t1->base == FCT && Pfct(t1)->memof
                || r2 == 'P' && t2->memptr() || t2->base == FCT && Pfct(t2)->memof) {
                if (r1 == 'Z') {
                    e1 = zero;
                    if (e2->base == ILIST)
                        e2 = e2->e1->e2;
                    ...
                    if (e1->base == ILIST)
                        ...
                        e1 = make_dot(e1, tbl);
                } else if (r2 == 'P' && !t2->memptr() || r1 == 'P' && !t1->memptr()) {
        ...
        case QUEST: {
            ...
            r2 = t2->num_ptr(b);
            suppress_error--;
            if ((r1 == 'P' && t1->memptr() || r1 == FCT && Pfct(t1)->memof && Pfct(t1)->f_static == 0)
                && (r2 == 'P' && t2->memptr()
                    || r2 == FCT && Pfct(t2)->memof && Pfct(t2)->f_static == 0)) {
            ...
            if (t1 == t2 || c1 && c2 && c1->tp == c2->tp || eo1 && eo2 && eo1->tp == eo2->tp)
                t = t1;
            else {
                if (c1 && c2) {
                    Ptype tt;
                ...
                if (r1 == 'P' && r2 == 'P') {
                    ...
                    Pptr p2 = t2->is_ptr();
                    Ptype tt;
                    if ((c1 = p1->typ->is_cl_obj()) && (c2 = p2->typ->is_cl_obj())) {
                        Pclass cl1 = Pclass(c1->tp);
                        Pclass cl2 = Pclass(c2->tp);
                        if (same_class(cl1, cl2) || cl2->has_base(cl1)) {
                            t = t1;
            ...
            caca:
                ...
                if (t != t1 && t->check(t1, 0)) {
                    ...
                    if (redo1) {
                        ...
                        e1 = new cast(t, e1);
                        if (cn)
                            e1->e1 = cast_cptr(Pclass(cn->tp), e1->e1, tbl, e1->base == CAST ? 1 : 0);
                    }
                }
                ...
                if (t != t2 && t->check(t2, 0)) {
                    ...
                    if (redo2) {
                        ...
                        e2 = new cast(t, e2);
                        if (cn)
                            e2->e1 = cast_cptr(Pclass(cn->tp), e2->e1, tbl, e2->base == CAST ? 1 : 0);
                    }
                }
        ...
        case ASSIGN:
            ...
            switch (e1->base) {
                ...
                case ASRS: {
                    ...
                    nin++;
                    if (e1->e1->not_simple())
                        error('s', "lvalue %k too complicated", b);
                    nin--;
                ...
                case REF: {
                    ...
                    // transformed into
                    //	((t=f()),&t)->i = j
                    if (r->e1->base == G_CM && r->e1->e2->base == G_ADDROF
            ...
            switch (t1->base) {
                ...
                case SHORT: {
                    ...
                    // no break
                case LONG:
                    if (b == ASSIGN && Pbase(t1)->b_unsigned && e2->base == UMINUS
                        && e2->e2->base == ICON)
            ...
            // error('d',"check(%t,%t) -> %d",e1->tp,t2,try_to_coerce(t1,e2,"assignment",tbl));
            {
                ...
                if (t1->base == COBJ && t1->classtype() && t1->classtype()->has_const_mem())
                    error("cannot assign to an object of class %t with const member(s)",
                          t1->classtype());
