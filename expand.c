...
Pname dcl_local(Ptable scope, Pname an, Pname fn, bit tempflag = 0) {
    ...
    if (an->n_stclass == STATIC) {
        if (an->tp->base != FCT && an->tp->base != OVERLOAD)
            error('s', &fn->where, "cannot expand inlineF with static%n", an);
        return an;
    }
    ...
    if (r) {
        ...
        if (r->tp->check(nx->tp, 0))
            error('s', &fn->where,
                  "cannot expand inlineF %a with two local variables with the sameN (%s)", fn,
                  an->string);
        else
            r = 0;
    }
...
st:
    ...
    switch (t1->base) {
        ...
        case COBJ: {
            ...
            if (Pclass(nn->tp)->csu == UNION)
                return 0;

            if (t2->base == COBJ && nn->tp == Pbase(t2)->b_name->tp)
                return 0;
...
{
    if (this == 0)
        error('i', "0->S::expand() for%n", expand_fn);
    Pstmt ostmt = Cstmt;
    if (where.line)
        Cstmt = this;
    ...
    if (expand_tbl) { /* make expression */
        ...
        if (memtbl) { // temporaries
            ...
            for (Pname n = memtbl->get_mem(i = 1); n; NEXT_NAME(memtbl, n, i)) {
                ...
                if (n->base != NAME || n->tp == any_type || n->n_key == CLASS)
                    continue;

                if (base == BLOCK && memtbl->real_block == this
                    && n->lex_level < 2
                    //&& n->n_table == Pfct(expand_fn->tp)->body->memtbl
                    && (n->string[0] != '_' // promoted from called
                                            // inlines
                        || n->string[1] != '_' || (n->string[2] != 'X' && n->string[2] != 'K'))) {
        ...
        switch (base) {
            ...
            case RETURN:
                ...
                if (e == 0)
                    ...
                    if (tt == 0)
                        tt = Pfct(expand_fn->tp)->returns;
                    // error('d',"return::expand() -- ee==%k  tt==%t",ee?ee->base:0,tt);
                    // display_expr(ee);
                    if (tt != ee->tp && ck_cast(tt, ee->tp))
                        ee = new cast(tt, ee);
                }
            ...
            case IF: {
                ...
                ret_seen = 0;
                ee = Pexpr(s->expand());
                if (ee->tp->memptr() || ee->base == ASSIGN && ee->e1->tp && ee->e1->tp->base != PTR)
                ...
                if (!has_return(s) && !has_return(else_stmt) && qq->e1->tp && qq->e2->tp
			...
			if (qq->e2->tp && !qq->e2->tp->check(void_type, 0)) {
                ...
                // display_expr(qq);
                if (c1 == 0 && c2 == 0 && t1 && t2 && t2->check(t1, ASSIGN) && t1->check(t2, ASSIGN)) {
                    z1 = makeCM(t1);
                    z2 = makeCM(t2);
                    if ((z1 && ret1) || (z2 && ret2))
                ...
                if (t1 && z1 == 0) {
                    ...
                    // of itself
                    z1 = t1->is_ptr_or_ref() && t2 && t2->is_ptr_or_ref() && t1->check(t2, 0)
                         && !const_problem;
                }
        ...
        case ICALL:
            if (expand_tbl && e1 == 0) {
                ...
                if (f->f_expr == 0
                    ...
                    error('s', &fn->where,
                          "cannot expand value-returning inline%a with call of non-value-returning "
                          "inline%a",
                          expand_fn, fn);
                //	else
...
*/
{
    ...
    switch (base) {
        default:
            return 2;
        case NAME:
            if (nin == 0 && Pname(this)->n_table == gtbl)
                return 2; // unsafe: aliasing
        case ZERO:
        case IVAL:
        // case FVAL:
        case ICON:
...
*/
{
    ...
    if ((body == 0 && f_expr == 0)               // before defined
        ...
        if (warning_opt)
            error('w', "cannot inline%n in thisE", fn);
    ...
    int ns = 0;
    for (Pname an = f_args; an; an = an->n_list)
        ns++;
    il->fct_name = fn;
    il->i_slots = ns;
    ...
    for (Pname n = at; n && ll; n = n->n_list, i++) {
        ...
        */
        ...
        for (Pname m = at; m; m = m->n_list) {
            ...
            if (n != m) {
                ...
                if (p = m->tp->is_ptr_or_ref())
                    if (p->check(n->tp, 0) == 0 || p->typ->check(n->tp, 0) == 0)
                        goto zxc;
            }
        }
        ...
        if (notsimple == 0 && ::watch_out && n->n_used > 1 && ee->base != G_ADDROF)
            goto zxc;
        if (n->n_assigned_to == FUDGE111 && ee != zero && notsimple == 0) {
            if (ee && ee->e1 && (ee->e1->base == NAME) && (!strcmp(ee->e1->string, "this")))
                goto zxc;
            ...
            zxc:
                if (last_expanded && last_expanded != dummy && last_expanded == curr_expr
                    && last_stmt == stmtno) {
                    if (warning_opt)
    ...
    if (f_expr) { // generate comma expression
        ...
        if (not_simple) {
            ...
            for (i = 0; i < il->i_slots; i++) {
                ...
                if (n->n_used || n->n_assigned_to || n->n_addr_taken) {
                    Pptr p1 = n->tp->is_ptr();
