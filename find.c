...
*/
{
    ...
    for (Pbcl b = start->baselist; b; b = b->next) {
        if (b->bclass->has_friend(f))
            return 1;
        if (friend_check(b->bclass, stop, f))
            return 1;
    }
    return 0;
}
...
*/
{
    ...
    if (::same_class(f->memof, this))
        return 1; // could be hit in recursive call
    ...
    if (fn) {
        ...
        for (Pcons p = templ_friends; p; p = p->cdr) {
            ...
            if (ptb && ptb->has_friend(fn))
                return 1;
        }
    }

    if (f->memof)
        return has_friend(f->memof);
    return 0;
}
...
*/
{
    ...
    if (cl->is_templ_instance()) {
        ...
        for (Pcons p = templ_friends; p; p = p->cdr) {
            Ptempl_base ptb = Ptempl_base(p->car);
            if (ptb && ptb->has_friend(tn))
                return 1;
        }
    }

    if (cl->in_fct)
        return has_friend((Pfct) cl->in_fct->tp);
    if (cl->in_class)
        return has_friend(cl->in_class);
    return 0;
}
...
static int is_accessible(Pname n, Pclass this_class, bit noCdcl = 0)
// 0 means n is not accessible to this_class
{
    ...
    if (n->n_protect && tcl) {
        if (mec) {
            if (tcl->has_friend(mec)
                || (mec->has_base(this_class) && (same_class(tcl, mec) || tcl->has_base(mec))))
                return 1;
        }
    ...
    if (Cdcl && Cdcl->base == NAME && Cdcl->n_stclass == STATIC &&
        //		Cdcl->n_initializer &&
        (!Cdcl->tp || Cdcl->tp->skiptypedefs()->base != FCT) && Cdcl->n_qualifier) {
        Pbase bn = Pbase(Cdcl->n_qualifier->tp);
        Pclass cl = Pclass(bn->b_name->tp);

        if (same_class(cl, this_class) || (n->n_protect && cl->has_base(this_class))
...
*/
{
    ...
    if (n == 0) { // swann's way? the guermantes' way? there must be a better way!
        if (in_class && templ_base == VANILLA && in_class->class_base == INSTANTIATED) {
    ...
    if (n) {
        ...
        if (n->base == PUBLIC) {
            if (mex && n->n_scope == 0) {
                if (is_accessible(n, this) == 0) {
                    if (me == dummy_fct) {
                        if (mec)
                            error("%t cannot access%n: %sM", mec, n,
                                  n->n_protect ? "protected" : "private");
                        else
                            error("G scope cannot access%n: %sM", n,
                                  n->n_protect ? "protected" : "private");
        ...
        if (cl == 0 || ::same_class(cl, this)) {
            if (mptr == 0 && n->n_stclass != STATIC && n->n_stclass != ENUM) {
                Ptype t = n->tp;
                ...
                if (mex && n->n_scope == 0 && n->tp->base != OVERLOAD) {
                    if (is_accessible(n, this) == 0) {
                        if (me == dummy_fct) {
                            ...
                            error("%n cannot access%n: %sM", me, n,
                                  n->n_protect ? "protected" : "private");
                        mex = 0; // suppress further error messages
                    }
                }
    ...
    if ((cl == 0 || ::same_class(cl, this)) // not qualified to a base class
        ...
        if (baselist == 0 || baselist->bclass->obj_size != obj_size) {
            Pname cn = this->k_tbl->find_cn(string); // SYM
            if (cn == 0)
                error('i', "CN %s missing inCdef::find_name(%s)", string, s); // SYM
            if (cn->tp->base == COBJ)
...
Pexpr classdef::find_in_base(char *s, Pclass cl, int access_only, int newflag) {
    ...
    for (Pbcl b = baselist; b; b = b->next) {
        ...
        if (ee) {
            ...
            if (e) {
                ...
                while (ex->base == MDOT || (ex->base == REF && ex->e1 == cc->c_this)) {
                    if (ex->base == MDOT)
                        evb += int(ex->i1);
                    ex = ex->mem;
                }
                ...
                while (eex->base == MDOT || (eex->base == REF && eex->e1 == cc->c_this)) {
                    if (eex->base == MDOT)
                        eevb += int(eex->i1);
                    eex = eex->mem;
                }

                // error('d', "find_in_base: ex: %k eex: %k, eevb: %d evb: %d", ex->base, eex->base,
                // eevb, evb ); relying on simple counts for sub-object identification isn't good enough

                if (ex != eex) {
                    // error('d',"ex %k tp %k eex %k tp %k", ex->base, ex->tp->base, eex->base,
                    // eex->tp->base );
                    //	if (!mqua) {
                    Pclass ocl, ncl;
                    if (ex->tp->base == FCT)
                        ocl = Pfct(ex->tp)->memof;
                    else if (ex->tp->base == OVERLOAD)
                        ocl = Pfct(Pgen(ex->tp)->fct_list->f->tp)->memof;
                    else
                        ocl = Pclass(ex->n_table->t_name->tp);
                    //						else ocl = 0;

                    if (eex->tp->base == FCT)
                        ncl = Pfct(eex->tp)->memof;
                    else if (eex->tp->base == OVERLOAD)
                        ncl = Pfct(Pgen(eex->tp)->fct_list->f->tp)->memof;
                    else
                        ncl = Pclass(eex->n_table->t_name->tp);
                    //						else ncl = 0;
                    ...
                    if (eb == 0 && eeb == 0 && !newflag) {
                        ...
                        else if (eevb && ::same_class(rootClass, this) && b->ppp == PUBLIC && pM
                    ...
                    break;
                } else if (e_bc && bc_found_in && ::same_class(e_bc->bclass, bc_found_in->bclass)
                           && e_bc->base != bc_found_in->base) {
                    error("ambiguous%n: (%t both %s and %s)", ex, e_bc->bclass,
                          e_bc->base == VIRTUAL ? "virtual" : "nonvirtual",
                          bc_found_in->base == VIRTUAL ? "virtual" : "nonvirtual");
                    e_bc = 0; // don't let it repeat
                }

                if (eevb && ::same_class(rootClass, this) && b->ppp == PUBLIC && pM
            ...
            if ((ee->tp->base == FCT && Pfct(ee->tp)->f_static && cl && Pfct(ee->tp)->memof
                 && strcmp(cl->string, Pfct(ee->tp)->memof->string) == 0)
                || (ee->base == NAME && Pname(ee)->n_stclass == STATIC))
                statflag = 1;
        }
    }
    ...
    if (mex) {
        if (bc->ppp == PRIVATE || bc->ppp == PROTECTED) {
            if (::same_class(this, mec) || (mec && has_friend(mec)) || (mef && has_friend(mef))
                || (mec && bc->ppp == PROTECTED && mec->is_base(string) && ppbase != PRIVATE))
                ;
            else if (bc->base == VIRTUAL && !::same_class(rootClass, this)) {
                // if one instance is public, it dominates
                if (pubVClass == 0 || strcmp(pubVClass->bclass->string, bc->bclass->string)) {
                ...
                if (me == dummy_fct) {
                    if (mec)
                        error("%t cannot access %s: %s is a %k BC", mec, name_oper(s),
                              bc->bclass->string, bc->ppp);
                    else
                        error("G scope cannot access %s: %s is a %k BC", name_oper(s),
                              bc->bclass->string, bc->ppp);
                    ...
                    if (mec && (mef == 0 || !::same_class(mec, mef->memof)))
                        ...
                        error("%n cannot access %s: %s is a %k BC", me, name_oper(s),
                              bc->bclass->string, bc->ppp);
                }
                mex = 0;
            }
            ...
            if (bc->base == VIRTUAL) {
                if (pM && strcmp(bc->bclass->string, pM->bc->bclass->string) == 0) {
                    delete pM;
                    pM = 0;
                    pubVClass = bc;
                } // previous private member becomes public
                else
                    pubVClass = bc; // ignore subsequent private instances
            }
        }
    }
    ...
    if (e->base == NAME && Pname(e)->n_stclass == STATIC) { // static member
        Pname(e)->use();
        return e;
    }
    if (e->base != NAME)
...
int has_virt(Pclass cl) {
    if (cl->virt_count)
        return 1;
    for (Pbcl b = cl->baselist; b; b = b->next)
        if (b->bclass->virt_count || has_virt(b->bclass))
            return 1;
    return 0;
}
...
*/
{
    ...
    for (Plist gl = Pgen(nx->tp)->fct_list; gl; gl = gl->l) {
        ...
        if (n == nn) {
            ...
            if (Cdcl && Cdcl->base == NAME && Cdcl->n_stclass == STATIC && Cdcl->n_initializer
                && Cdcl->n_qualifier) {
                Pbase bn = Pbase(Cdcl->n_qualifier->tp);
                Pclass ccl = Pclass(bn->b_name->tp);

                if (same_class(ccl, cl) || ccl->has_friend(f) || (n->n_protect && ccl->has_base(ncl)))
                    return;
            }
