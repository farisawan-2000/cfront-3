        ...
        case NAME: {
            ...
            *on = *this;
            ...
            if (s && ansi_opt && !templp->in_progress)
                fprintf(out_file, "%s %s%s;\n", csu == UNION || csu == ANON ? "union" : "struct",
                        nt ? "__" : "", s);
            return tn;
        }
        default:
...
*/
{
    ...
    if (stc == STATIC && tp && tp->base == FCT && Pfct(tp)->f_const)
    ...
    if (tp // FUDGE: fix the bad grammar
        && tp->base == FCT && (n_oper == TNAME || Pfct(tp)->returns)) {
        Pfct f = Pfct(tp);
                ...
                case FCT: { //	Pexpr e = f2->argtype;
                    ...
                    ldld:
                        switch (ee->base) {
                            case DEREF: {
                                Ptype tt =
                                    (ee->e2) ? Ptype(new vec(0, ee->e2)) : Ptype(new ptr(PTR, 0));
                                if (t)
                                    Pptr(t)->typ = tt;
                                else
                                    tpx = tt;
...
zse1:
    ...
    switch (b->base) { //	separate class definitions
        ...
        case EOBJ:
            ...
            if (Penum(nn->tp)->e_body == 2) {
                if (tp && tp->base == FCT) {
                    error(&this->where, "enum%n defined as returnT for%n (did you forget a ';'?)", nn,
                          this);
                    nn = this;
                    break;
                }
    ...
    skipp:
        ...
        if (f->base != FCT) {
            ...
            if (inli)
                error("inline nonF%n", n);
            if (virt)
                error("virtual nonF%n", n);
        ...
        */
        {
                    ...
                    if (nn->base != NAME) {
                        if (!clob) {
                            if ((f->returns->base == TYPE)
                                && (Pbase(f->returns)->b_name->n_template_arg
            ...
            is_obj:
                ...
                if (f->argtype->base != ELIST)
                    f->f_args = f->argtype = (Pname) new expr(ELIST, (Pexpr) f->argtype, 0);
                if (n->n_initializer) {
                    ...
                    n->n_initializer =
                        new texpr(VALUE, cn ? cn->tp : template_formal_type, (Pexpr) f->argtype);
                }
                goto ok;
            zzz:
...
Ptype vec::normalize(Ptype vecof) {
    ...
    switch (t->base) {
        case PTR:
        case RPTR:
            return Pptr(t)->normalize(this);
        case VEC:
            return Pvec(t)->normalize(this);
        case FCT:
            return Pfct(t)->normalize(this);
        default:
            error('i', "bad arrayT(%d)", t->base);
            return 0;
    }
}
...
Ptype ptr::normalize(Ptype ptrto) {
            ...
            if (memof)
                if (Pfct(ptrto)->memof) {
                    if (!same_class(memof, Pfct(ptrto)->memof))
                        error("P toMF mismatch: %s and %s", memof->string, Pfct(ptrto)->memof->string);
                } else
    ...
    if (t == 0) {
        Pbase b = Pbase(ptrto);
        if (Pfctvec_type && b_const == 0 && b->b_unsigned == 0 && b->b_const == 0 && bc == 0
            && memof == 0 && ptname == 0 && base == PTR) {
            switch (b->base) {
                case INT:
                    delete this;
                    return Pint_type;
...
{
    Pname n;
    /*fprintf(stderr,"%d argtype %d %d dcl %d %d\n",this, argtype, argtype?argtype->base:0, dcl,
     * dcl?dcl->base:0); fflush(stderr);*/
    switch (base) {
        case FCT:
            break;
        case ANY:
...
static Pname k_find_in_base(char *s, Pclass cl, Pclass &acl, TOK k)
...
Pname k_find_name(char *s, Pktab tbl, TOK k)
/* parse table lookup
 */
{
recurse:
    ...
    if (tbl->k_id == CLASS) {
        ...
        n = tbl->look(s, 0);
    if (n && k == HIDDEN && n->base != TNAME)
        n = n->n_hidden;
    DB(if (Kdebug >= 3) {
        if (n)
            error('d', "found%n base%k tp%t in %s", n, n->base, n->tp, n->n_ktable->whereami());
        else
            error('d', "%s not found in %s", s, tbl->whereami());
    });
    if (n)
        return n;
...
Pname k_find_member(char *s, Pclass cl, TOK k)
/* parse table lookup
 */
{
    ...
    DB(if (Kdebug >= 3) {
        if (n)
            ...
            error('d', "%s not found in bases of %s", s, tbl->whereami());
    });
    return n;
}

// SYM -- table insertion routines...
Pname insert_name(Pname nn, Pktab tbl)
 ...
 */
{
    ...
    if (nx && nx->n_ktable == tbl) {
        if (nx->base == TNAME) {
            ...
            if (nx->tp && nx->tp->base != COBJ && nx->tp->base != EOBJ) {
                DB(if (Kdebug >= 2) error('d', "hidingTdef%n", nx));
            }
            nx->n_key = HIDDEN;
            Pname ny = tbl->insert(nn, 0);
            if (Nold)
                error("hiddenTN%n inserted twice", nx);
            else
                ny->n_hidden = nx;
            nx = ny;
...
Pname insert_type(Pname nn, Pktab tbl, TOK tcsue)
 ...
 */
{
    ...
    DB(if (Kdebug >= 2)
           error('d', "insert_type( nn%n, tbl(%d) %s, tcsue%k )", nn, tbl, tbl->whereami(), tcsue););
    if (tbl->k_id == ARG) {
        // types defined in arg lists are entered in surrounding scope
        do
            tbl = tbl->k_next;
        while (tbl->k_id == ARG);
        if (tbl->k_id == TEMPLATE)
            tbl = tbl->k_next;
    }
    if (tbl->k_id == CLASS) {
        if (ccl == 0)
            error('i', &nn->where, "insert_type(%n) -- no ccl for class table!", nn);
    }
...
char *ktable::whereami() {
    if (this == 0)
        ...
        return "block scope";
    else if (k_id == TEMPLATE)
        return "template scope";
    else if (k_name)
        return k_name->string;
    else
        return "??? scope";
}
