...
int replace_ret(Pstmt sl, int level, Ptable tbl) {
    ...
    if (!level && nl) {
        ...
        ls->s_list = new lstmt(LABEL, curloc, lab, new estmt(SM, curloc, dummy, 0));
    }
...
Pname classdef::has_oper(TOK op) {
    ...
    if (n->tp->base == FCT && Pname(n)->n_dcl_printed == 0)
        Pname(n)->dcl_print(0);
    return Pname(n);
}
...
*/
{
    ...
    for (Pstmt ss = s; ss; ss = ss->s_list) {
        ...
        switch (ss->base) {
            case BLOCK:
                if (Pblock(ss)->memtbl || is_expr(ss->s) == 0)
                    return 0;
            case SM:
            ...
            case IF:
                if (is_expr(ss->s) == 0)
                    return 0;
                if (ss->else_stmt && is_expr(ss->else_stmt) == 0)
                    return 0;
                break;
            default:
                return 0;
...
void name::simpl() {
    ...
    if (tp == 0)
        error('i', "%n->N::simple(tp==0)", this);
    if (tp->base == CLASS && n_key == REF)
        return;
    if (tp->base == CLASS && (Pclass(tp)->defined & DEFINED) == 0)
        return;
    ...
    switch (tp->base) {
        ...
        case OVERLOAD: {
            for (Plist gl = Pgen(tp)->fct_list; gl; gl = gl->l)
                gl->f->simpl();
            break;
        }
        ...
        case FCT: {
            ...
            if (th) {
                ...
                if (th->n_assigned_to && th->n_assigned_to != FUDGE111)
                    error('w', &where, "assignment to this (anachronism)");
            }

            if (tp->defined & SIMPLIFIED)
                return;
            if ((tp->defined & DEFINED) == 0)
                return;
            ...
            if (f->body) {
                ...
                if (f->f_inline == 0 || debug_opt)
                    ...
                    for (n = f->f_args; n; n = n->n_list) {
                        n->base = ANAME;
                        n->argno = i++;
                        // ??				if (n->n_table != scope) error('i',"aname scope: %d %n
                        // %d %d\n",n,n,n->n_table,scope);
                    }
...
Pexpr call_ctor(Ptable tbl, Pexpr p, Pexpr ctor, Pexpr args, int d, Pexpr vb_args) {
    ...
    if (args && args->base != ELIST)
        args = new expr(ELIST, args, 0);
...
Pexpr call_dtor(Pexpr p, Pexpr dtor, Pexpr arg, int d, Pexpr vb_args) {
    ...
    if (f->memof->c_body == 1)
        f->memof->dcl_print(0);
    if (Pname(dtor)->n_dcl_printed == 0)
        Pname(dtor)->dcl_print(0);
...
Pstmt fct::dtor_simpl(Pclass cl, Pexpr th) {
    ...
    Pname a = f_args;
    if (a == 0 || a->n_list == 0)
        error('i', "__freeA missing in destructor for %t", cl);
    Pname p = 0;
    ...
    for (Pname m = tbl->get_mem(i); m; NEXT_NAME(tbl, m, i)) {
        if (m->base == TNAME || m->tp == 0)
            continue;

        if (m->n_stclass == STATIC || m->base == PUBLIC)
            continue;
        ...
        if (dtor) {
            ...
            if (b->base == VIRTUAL) {
                ...
                for (Pname dd = f_this->n_list; dd != argtype; dd = dd->n_list)
                    if (strcmp(dd->string, bcl->string) == 0)
                        break;
...
Pexpr classdef::get_vptr_exp(char *s) {
    ...
    if (c_body == 1)
        b->bclass->dcl_print(0);
    if (c_body == 3)
        b->bclass->print_all_vtbls(b->bclass);
    if (b->base == VIRTUAL) {
        vp->i1 = 1;
...
int fct::ctor_simpl(Pclass cl, Pexpr th) {
    ...
    */
    ...
    for (Pbcl l = cl->baselist; l; l = l->next) {
        ...
        for (Pname dd = f_this->n_list; dd != argtype; dd = dd->n_list)
            if (strcmp(dd->string, bc->string) == 0)
                break;
        ...
        // reverse init order
        if (dp)
            init_list = init_list ? new expr(CM, dp, init_list) : dp;
    }
        ...
        for (Pbcl l = cl->baselist; l; l = l->next) {
            ...
            switch (i->base) {
                ...
                case G_CALL: {
                    ...
                    if (l == cl->baselist && cl->baselist->next == 0)
                        i = new expr(ASSIGN, th, cc);
                    break;
                }
                default:
                    error('i', "badBCIr %k", i->base);
        ...
        if (no_of_returns) {                                 // could be OK
            ...
            if (tt)
                switch (tt->base) {
                    ...
                    case IF:
                        if (tt->base == IF && tail->base == IF && tt->else_stmt == 0
                            && no_of_returns == 1) {
            ...
            //		error(Pfct(curr_fct->tp)->returns->is_cl_obj()?0:'w',"no value returned
            //from%n",curr_fct); 	else if (warning_opt) 		error('w',"no value returned from%n",curr_fct);
            if (Pfct(curr_fct->tp)->f_inline && Pfct(curr_fct->tp)->returns->skiptypedefs() != void_type
                ...
                if (Pfct(curr_fct->tp)->is_templ() && Pfct(curr_fct->tp)->returns == defa_type)
                    error("no value returned from%n", curr_fct);
                else if (Pfct(curr_fct->tp)->returns == defa_type && !Pfct(curr_fct->tp)->memof
                         && !curr_fct->n_oper && simpl_friend == 0)

                    error(strict_opt ? 0 : 'w', "no value returned from%n (anachronism)", curr_fct);
                else
                    error("no value returned from%n", curr_fct);
            }
            if (warning_opt && strcmp(curr_fct->string, "main") == 0
