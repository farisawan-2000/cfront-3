...
static int is_empty(Pclass cl, bit const_chk = 0) { /*
    ...
    if (const_chk == 0 && (cl->baselist != 0 || mbr_cnt > 1))
        return 0;
    ...
    for (Pname nn = cl->memtbl->get_mem(i); nn; NEXT_NAME(cl->memtbl, nn, i)) {
        if (nn->base == NAME && nn->n_anon == 0 && nn->tp->base != FCT && nn->tp->base != OVERLOAD
            && nn->tp->base != CLASS && nn->tp->base != ENUM && nn->tp->base != EOBJ
            && nn->n_stclass != STATIC) {
            if (nn->string[0] == '_' && nn->string[1] == '_' && nn->string[2] == 'W')
                return 1;
            else
                return 0;
        }
...
*/
{
    ...
    for (Pexpr il = init->e1; il; il = il2) {
        ...
        if (sti_tbl == tbl) {
            ...
            //			if (il->e1->base==VALUE && il->e1->e1==0 && i!=0) { // no explicit
            //initializer 				el = new texpr(NEW,n->tp,0); 				el = el->typ(tbl); 				el->simpl();
            //			}
            //			else
    ...
    switch (e->base) {
        ...
        case 0: // hack for `new type (expr)'
            if (tbl) {
                ...
                if (e->tp && e->tp->base == VEC && e->base == NEW || e->base == GNEW)
...
static void check_def_name(Pname nn, int scope) {
    Pfct f = Pfct(nn->tp);
    if (def_name == 0 && pdef_name == 0 && friend_in_class == 0 && scope == EXTERN
        && nn->n_scope != STATIC && nn->n_oper != NEW && nn->n_oper != DELETE) {
...
static void export_anon(Pname un, Pclass cl, Ptable tbl) {
    if (tbl == gtbl) {
        if (un->n_sto != STATIC)
    ...
    for (Pname nn = mtbl->get_mem(i = 1); nn; NEXT_NAME(mtbl, nn, i)) {
        ...
        if (nn->n_key == HIDDEN) // was an error ...
            continue;
        if (nn->n_key == CLASS && nn->string[0] == '_' && nn->string[1] == '_' && nn->string[2] == 'C')
            continue;
        // error('d',&nn->where,"exporting anon unionM%n base%k tp%t
        // n_key%k",nn,nn->base,nn->tp,nn->n_key);
        ...
        if (n && n->n_table != tbl) {
            ...
            if (n->n_table->real_block != realtbl->real_block || n->lex_level != nn->lex_level)
                n = 0;
        }
        if (n == 0) {
        adef:
            n = tbl->insert(nn, nn->n_key);
        ...
        n->n_anon = un->string;
        nn->n_table = tb;
        if (cl->in_class && un->n_sto == STATIC && nn->base != TNAME && nn->n_key != CLASS)
            n->n_stclass = STATIC;
    }
}

void classdef::make_vec_ctor(Pname default_ctor) {
...
*/
{
    ...
    switch (base) {
        case TNAME:
            DB(if (Ddebug >= 1) error('d', &where, "dclTN%n%t scope%k", this, tp, scope););
            {
                ...
                if (tpdef) { // nested
                    Ptype tx = tpdef;
                    if (tx->in_class
                        && (tx->templ_base == BOUND_TEMPLATE || tx->templ_base == CL_TEMPLATE)) {
                ...
                // error('d',"      ll this %d nn %d tbl
                // %s",lex_level,nn?nn->lex_level:0,tbl==gtbl?"global
                // table":tbl->t_name?tbl->t_name->string:"???"); if(tpdef)error('d',"
                // in%t",tpdef->in_class);
        ...
        case NAME:
            DB(if (Ddebug >= 1) {
                ...
                error('d', &where, "  n_oper%k n_sto%k n_stclass%k n_scope%k", n_oper, n_sto, n_stclass,
                      n_scope);
            });
            switch (n_oper) {
                case COMPL:
    ...
    if (n_qualifier) { // c::f()
        ...
        if (tp->base == FCT) { // III
            if (friend_in_class == 0 && n_sto != FRIEND
        ...
        case EXTERN:
            switch (scope) {
                ...
                case FCT: {
                    ...
                    tp->dcl(tbl);
                    if (nn && tp->base != FCT && tp->check(nn->tp, 0)) {
    ...
    switch (tx->base) {
        ...
        case CLASS:
            if (n_key == REF) {                // class x;
                ...
                //			    char* su = (cl->csu==UNION || cl->csu==ANON) ? "union" :
                //"struct"; 			    if ( cl->nested_sig ) 				fprintf(out_file,"%s __%s;\n",su,cl->nested_sig); 			    else
                //if ( cl->lex_level == 0 ) 				fprintf(out_file,"%s %s;\n",su,cl->string); 			    else
        ...
        case FIELD:
            ...
            if (cc->not == 0 || (cc->cot->csu == UNION && !ansi_opt) || cc->cot->csu == ANON) {
                if (cc->not )
        ...
        case COBJ: {
            ...
            //			error('i',"'%n'->dcl(%d,%k) C%t is not visible in this
            //scope",this,tbl,scope,cl);

            if (cl->csu == ANON)
                export_anon(this, cl, tbl);
        ...
        cde:
            ...
            if (Nold) {
                ...
                ext_fudge:
                    ...
                    if (n_sto == 0 && nn->n_sto == EXTERN && n_initializer && tp->tconst())
                        if (vec_const == 0)
                            n_sto = EXTERN;
                    ...
                    switch (scope) {
                        ...
                        case PUBLIC:
                            ...
                            return 0;
                        case EXTERN:
                            if (n_sto == 0 || (n_sto == EXTERN && n_initializer)) {
                                switch (nn->n_sto) {
                                    case 0:
                                    ...
                                    case EXTERN:
                                        ...
                                        if (nn->n_stclass == STATIC
                                            && (nn->n_scope == PUBLIC || nn->n_scope == 0)) {
                ...
                if (tp->base == VEC) { // handle:	extern v[]; v[200];
                    ...
                    if (Pvec(ntp)->size) {
                        if (Pvec(tp)->size && Pvec(ntp)->size != Pvec(tp)->size)
                            error("bad array size for%n: %d %dX", this, Pvec(tp)->size,
                                  Pvec(ntp)->size);
                    } else
                        Pvec(ntp)->size = Pvec(tp)->size;
                }
            } else {
                if (scope != ARG && n_sto != EXTERN
                    && (n_sto != STATIC || scope != 0 && scope != PUBLIC) // static member
                    && n_initializer == 0 && tp->skiptypedefs()->base == VEC
                    && Pvec(tp->skiptypedefs())->size == 0)
                    if (Pvec(tp)->dim == 0)
                        error(&where, "dimension missing for array%n", this);

                if (scope == EXTERN && n_sto == 0 && tp->is_const_object())
                ...
                switch (t->base) {
                    ...
                    default:
                        ...
                        switch (nn->n_stclass) {
                            ...
                            case STATIC:
                                if (n_sto != EXTERN) {
                                    if (nn->n_scope && nn->n_scope != PUBLIC)
                                        t->tsizeof(); // check that size is known
                                    else              // is this a static mem def?
                                        if (tbl->t_name == 0 || tbl == gtbl
                                            || !same_class(Pclass(tbl->t_name->tp),
                                                           Pclass(nn->n_table->t_name->tp)))
                                        t->tsizeof(); // check that size is known
                                }
                        }
                        fake_sizeof = 0;
            ...
            lll:
                switch (t->base) {
                            ...
                            switch (nn->n_scope) {
                                ...
                                dtdt:
                                    ...
                                    if (nn->lex_level && nn->n_sto == STATIC) {
                                        if (ctor == 0)
                                            error('s', "local static%n has%n but noK(add%n::%n())", nn,
                                                  dtor, cn, cn);
                                        goto static_init;
                                    }
                        ...
                        if (ctor) {
                            ...
                            switch (nn->n_scope) {
                                ...
                                default:
                                    if (vec_seen && init) {
                                        if (1 < vec_seen)
                                            error('s', "Ir for multi-dimensional array%n ofOs ofC%tWK",
                                                  this, cl);
                                        else {
                                            if (sti) {
                                                if (sti_tbl == 0)
                                ...
                                case 0:
                                    if (n_stclass == STATIC) { // III
                                        if (vec_seen && n_initializer) {
                                            if (1 < vec_seen)
                                                error('s',
                                                      "Ir for multi-dimensional array%n ofOs ofC%tWK",
                                                      this, cl);
                                            else {
                                                if (sti_tbl == 0)
                                                    sti_tbl = new table(8, gtbl, 0);
                                    ...
                                    if (vec_seen) {
                                        ...
                                        if (c == 0)
                                            ...
                                            Pname ctor = cl->has_ctor();
                                            if (ctor
                                                && (!tbl->t_name || !tbl->t_name->tp
                                                    || tbl->t_name->tp->base != CLASS)) {
                            ...
                            if (init) {
                                ...
                                inin:
                                    init = init->typ(tbl);
                                    if (init->base == G_CM && nn->tp->check(init->tp, 0) == 0)
                            ...
                            if (stct) {
                                if (tbl != gtbl && nn->n_sto == EXTERN) {
                                    ...
                                    if (init->base == DEREF && init->e1->base == G_CALL)
                                        init->base = STAT_INIT;
                                    else {
                                    ...
                                    if (tp->base == VEC && Pvec(tp)->size) {
                                        if (1 < vec_seen)
                                            error('s',
                                                  "implicit staticIr for multi-dimensional array ofOs "
                                                  "ofC%tWK",
                                                  cl);
                                        else {
                                            Pexpr ilist = 0;
                            ...
                            init = init->typ(tbl);
                            if (nn->tp->check(init->tp, 0) == 0 && init->base == G_CM)
                                (void) replace_temp(init, nn->address());
                            else
                                goto str;
                        } else if (init->base == ILIST) { // class or union
                            error("cannotI%nWIrL because class %s has private or protected members", nn,
                                  cl->string);
                    ...
                    str:
                        ...
                        const_save = const_save || n_scope == ARG || (t->tconst() && vec_const == 0);
                        ...
                        switch (init->base) {
                                ...
                                if (v->base == VEC) {
                                    ...
                                    if (vv->base == CHAR) {
                                        ...
                                        if (sz == 0)
                                            ...
                                            error(&where, "Ir too long (%d characters) for%n[%d]", isz,
                                                  nn, sz);
                                        break;
                                    }
                                }
                            ...
                            dfdf : {
                                ...
                                switch (nt->base) {
                                    ...
                                    case EOBJ:
                                        ...
                                        // no break
                                    case LONG: {
                                        if (Pbase(nt)->b_unsigned && init->base == UMINUS
                                            && init->e2->base == ICON)
                                            ...
                                            switch (it->base) {
                                                ...
                                                case LONG:
                                                    if (nt->tsizeof() < it->tsizeof()) {
                                                        Pexpr tmp = new texpr(G_CAST, nt, init);
                                                        init = tmp->typ(tbl);
                                                    }
                                            }
                                        }
                                ...
                                if (c1 && c2 && Pclass(c2->tp)->has_base(Pclass(c1->tp))) {
                            ...
                            stgg:
                                ...
                                if (init && n_stclass == STATIC
                                    && (sti_vb || (need_sti(init) && check_static_pt(nn)))) {
...
char *make_nested_name(char *s, Pclass cl) { // Q<cnt>_<class_names><space><null>
    ...
    for (Pclass nc = cl; nc && nc->lex_level == cl->lex_level; nc = nc->in_class) {
        if (cnt > nested_depth - 1)
            error('s', "nested depth class beyond %d unsupported", nested_depth);
