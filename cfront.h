...
int error(loc *, const char *, const ea & = *ea0, const ea & = *ea0, const ea & = *ea0,
          const ea & = *ea0);
int error(int, loc *, const char *, const ea & = *ea0, const ea & = *ea0, const ea & = *ea0,
          const ea & = *ea0);
extern char emode;
...
enum Templ_type { VANILLA, FCT_TEMPLATE, CL_TEMPLATE, BOUND_TEMPLATE, INSTANTIATED, UNINSTANTIATED };
...
struct virt : public node {
    ...
    bit printed;
    virt(Pclass cl, velem *v, char *s, bit flag, int ni) {
        base = XVIRT;
        vclass = cl;
        virt_init = v;
        string = s;
...
struct gen : public type { // OVERLOAD
    ...
    int has_templ() {
        return holds_templ != no_templ;
    }
    int pure_templ() {
        return holds_templ == all_templ;
    }
};
#endif
...
inline Pptr type::addrof() {
    return new ptr(PTR, this);
}

inline bit type::is_or_pts_to(TOK k) {
...
struct texpr : public expr { // G_CAST, CAST NEW VALUE (also ICALL)
    texpr(TOK bb, Ptype tt, Pexpr ee) : expr(bb, ee, 0) {
        tp2 = tt;
    }
};

struct cast : public expr { // G_CAST
    cast(Ptype tt, Pexpr ee) : expr(G_CAST, ee, 0) {
        tp = tp2 = tt;
    }
};

struct ival : public expr { // IVAL
    ival(long ii) : expr(IVAL, 0, 0) {
        i1 = ii;
    }
};
...
struct qexpr : public expr { // QUEST	cond ? e1 : e2
    qexpr(Pexpr ee, Pexpr ee1, Pexpr ee2) : expr(QUEST, ee1, ee2) {
        cond = ee;
    }
};

struct ref : public expr { // REF DOT	e1->mem OR e1.mem
    ref(TOK ba, Pexpr a, Pexpr b) : expr(ba, a, 0) {
        mem = b;
    }
};

struct mdot : public expr { // MDOT		a.b
    mdot(char *a, Pexpr b) : expr(MDOT, 0, 0) {
        string2 = a;
        mem = b;
    }
};
...
struct name : public expr { // NAME TNAME DTOR and the lexical keywords
    ...
    bit is_template_arg() {
        return n_template_arg == template_type_formal;
    }
    void del();
...
struct stmt : public node { /* BREAK CONTINUE DEFAULT */
    ...
    bit has_expr() {
        return (base != BLOCK && base != PAIR) ? e != 0 : 0;
    }
#endif
    int discriminator(int); // union discriminator fcn
};
...
struct estmt : public stmt /* SM WHILE DO SWITCH RETURN CASE */
                           ...
                           */
{
    estmt(TOK t, loc ll, Pexpr ee, Pstmt ss) : stmt(t, ll, ss) {
        e = ee;
    }
};
...
{
    ifstmt(loc ll, Pexpr ee, Pstmt ss1, Pstmt ss2) : stmt(IF, ll, ss1) {
        e = ee;
        else_stmt = ss2;
    };
};
...
struct lstmt : public stmt /* LABEL GOTO */
                           ...
                           */
{
    lstmt(TOK bb, loc ll, Pname nn, Pstmt ss) : stmt(bb, ll, ss) {
        d = nn;
    }
};

struct forstmt : public stmt { // FOR
    forstmt(loc ll, Pstmt fss, Pexpr ee1, Pexpr ee2, Pstmt ss) : stmt(FOR, ll, ss) {
        for_init = fss;
        e = ee1;
        e2 = ee2;
    }
...
struct pair : public stmt { // PAIR
    pair(loc ll, Pstmt a, Pstmt b) : stmt(PAIR, ll, a) {
        s2 = b;
    }
};
#endif
...
struct con_dtor {
    ...
    Pexpr condition;
    con_dtor *next;
    con_dtor(Pname n, Pexpr e) : tn(n), condition(e), next(0) {
    }
};
...
// encapsulate casts
inline Pname type::bname() {
    return Pbase(this)->b_name;
}
inline Ptype type::bname_type() {
    return Pbase(this)->b_name->tp;
}
...
inline bit type::is_templ_instance() {
    Templ_type t = (base == FCT) ? Pfct(this)->fct_base
                                 : (base == CLASS) ? Pclass(this)->class_base : (Templ_type) 0;
    return (t == INSTANTIATED || t == UNINSTANTIATED);
}

#ifndef GRAM
inline Pfct name::get_fct() {
    return (tp->base == FCT) ? Pfct(tp) : Pfct(Pgen(tp)->fct_list->f->tp);
}

inline Pname table::insert_copy(Pname n, TOK k) {
    Pname nn = new name;
    *nn = *n;
    nn->n_table = 0;
    nn->n_tbl_list = 0;
