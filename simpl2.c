...
void expr::simpl() {
    ...
    switch (base) {
        ...
        case G_CM: {
            ...
            e1->simpl();
            e2->simpl();
            if (e1->base == ICALL && e1->e1 == 0)
                n = e1->il->fct_name;
            if (e2->base == ICALL && e2->e1 == 0)
                n = e2->il->fct_name;
            if (n)
                error('s', "cannot expand inline void%n called in commaE", n);
            return;
        }
        ...
        case ASSIGN: {
            ...
            if (th && th == e1 && curr_fct->n_oper == CTOR && init_list) {
                ...
                if (warning_opt) // timid
                    error('w', "assignment to ``this'' inK: try defining%t::operator new() instead",
                          f->memof);
            }
            break;
        }
    ...
    switch (base) {
        ...
        case OROR:
            if (Ntmp)
                error('s', "temporary ofC%n with destructor needed in%kE", Ntmp, base);
            // no break;
        default:
            Ntmp = 0;
    }
...
Pexpr vptr_entry(Pexpr pp, Pexpr ie, Pclass cl) {
    ...
    if (vp == 0) {
        ...
        if (cc->cot && (same_class(cc->cot->in_class, cl) || same_class(cc->cot, cl)))
            error('s',
                  "call of virtualF%n beforeC %s has been completely declared - %s make function "
                  "non-virtual",
                  vfunt_name, cl->string,
                  (curr_fct && strcmp("..", curr_fct->string) == 0)
                      ? "try moving call from argument list into function body or"
                      : "");
        else
            error('i', "can't find vptr");
    }
...
Pexpr new_this(Pexpr pp, Pexpr ee) {
    ...
    Ptype ct = pp->tp;
    if (pp->base == NAME && Pname(pp)->n_xref)
        ct = pp->tp->addrof();
...
*/
{
    ...
    if (cl->c_body == 1)
        cl->dcl_print(0); // look for first use of cl
...
*/
{
    ...
    if (f) {
        ...
        if (f->fct_base == INSTANTIATED && curr_fct && strcmp(curr_fct->string, fn->string) == 0)
    ...
    switch (e1->base) {
        case MEMPTR: // (p ->* q)(args)
        {
            ...
            if (f->f_this == 0) { // might not know about ``this'' yet
                if (f->memof == 0)
                    error('i', "memof missing");
                if (f->memof->class_base == CL_TEMPLATE && same_class(cl, f->memof))
                    f->memof = cl;
            ...
            nin = 1;
            if (q->not_simple())
                error('s', "2nd operand of .* too complicated");
            nin = 0;
            ...
            nin = 1;
            if (p->not_simple())
                error('s', "1st operand of .* too complicated");
            nin = 0;
...
*/
{
    ...
    switch (base) {
        ...
        case RETURN: { /*	return x;
            ...
            if (e == 0)
                e = dummy;
            if (e == dummy && curr_fct->n_oper == CTOR)
                e = f->f_this;
            ...
            if (f->f_result) { // ctor(_result,x); dtors; return;
                if (e->base == G_CM)
                    e = replace_temp(e, f->f_result);
                if (f->nrv && return_nrv(e))
                    e = dummy;
                e->simpl();
        ...
        case LABEL:
            if (del_list)
                error('s', "label in blockW destructors");
            s->simpl();
            break;
            ...
            */
            {
                ...
                if (n == 0)
                    error('i', &where, "label%n missing", d);
                ...
                xyzzy:
                    if (r == 0)
                        error('i', &where, "finding root of subtree");
                            ...
                            for (Pname n = p->get_mem(i = 1); n; NEXT_NAME(p, n, i)) {
                                ...
                                if (cln = n->tp->is_cl_obj()) {
                                    ...
                                    if (d && dtor) { //  __vec_delete(vec,noe,sz,dtor,0);
                                        Pfct f = Pfct(d->tp);
                                        int i = 0;
                                        for (Pname nn = f->f_args->n_list; nn && nn->n_list;
                                             nn = nn->n_list)
                                            i++;
                                        Pexpr a = cdvec(vec_del_fct, n, cl, d, 0, new ival(i));
                                        Pstmt dls = new estmt(SM, n->where, a, 0);
        ...
        case FOR: // "for (s;e;e2) s2; => "s; for(;e,e2) s2"
            ...
            if (e2) {
                ...
                if (e2->base == ICALL)
                    if (e2->e1 == 0)
                        error('s', "cannot expand inline void%n called in forE", e2->il->fct_name);
            }
    ...
    if (base != BLOCK && memtbl) {
        ...
        for (; tn; NEXT_NAME(memtbl, tn, i)) {
            ...
            if (cln = tn->tp->is_cl_obj()) {
                Pname d = Pclass(cln->tp)->has_dtor();
                if (d) { /* n->cl::~cl(0); */
                    if (need_lift && is_probably_temp(tn->string) && tmp_tbl->look(tn->string, 0))
                        continue;
        ...
        if (ss) {
            ...
            switch (base) {
                ...
                case SM: // place dtors after all "converted" DCLs
                    if (t1) {
                        //					ccheck(e);
                        for (Pstmt ttt, tt = this; (ttt = tt->s_list) && ttt->base == SM; tt = ttt)
                            ;
                        t2->s_list = ttt;
                        tt->s_list = ss;
                        // Cstmt = ostmt;
...
static Pexpr mk_new_with_args(Pexpr pe, Ptype tt, Pclass cl,
                              Pexpr vec = 0) { // allocate using operator new(sizeof(cl),args1)
    Pexpr p;
    Pexpr args = pe->e2;
...
*/
{
    ...
    if (init && init->base)
        init = 0; // only non-ctor init
                  // error('d',"simpl_new %k e1 %k e2 %k init %k",base,
                  // e1?e1->base:0,e2?e2->base:0,init?init->base:0);
    ...
    if (cl_obj_vec) {
        ...
        if (ctor == 0) {
            if (covn->has_ctor())
                error("new %s[], no defaultK", covn->string);
            if (covn->has_dtor() == 0)
                cl_obj_vec = 0;
        }
    }
...
xxx:
    ...
    if (cl_obj_vec) { // _vec_new(0,no_of_elements,element_size,ctor)
        ...
        Pexpr noe =
            (var_expr) ? (sz != 1) ? new expr(MUL, const_expr, var_expr) : var_expr : const_expr;
        const_expr = szof;
        ...
        if (ctor) {
            ...
            arg = new expr(ELIST, zero, 0);

        Pexpr sub = 0;
        if (e2 && e2->e1 && e2->e1->tp) { // new(size_t, args)
        ...
        arg = new expr(ELIST, e2 ? (sub ? sub : e2) : new texpr(G_CAST, Pvoid_type, zero),
                       arg); // may be preallocated
        base = G_CAST;
        tp2 = tp;
...
*/
{
    ...
    if (cln) {
        cl = Pclass(cln->tp);
        if ((cl->defined & DEFINED) == 0)
            error('w', "delete%t (%t not defined)", cl, cl);
    } else
        cl = 0;
    ...
    if (cl && ((n = cl->has_dtor()) || (e2 && cl->has_ictor()))) { // ~cl() might be virtual
        // xxx check for private/protected op delete
        {
            ...
            if (base != GDELETE) {
                ...
                if (n->tp->base == OVERLOAD || Pfct(n->tp)->nargs == 2) {
        ...
        if (n && n->n_scope != PUBLIC)
            check_visibility(n, 0, cl, cc->ftbl, cc->nof);
