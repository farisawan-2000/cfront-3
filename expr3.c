...
Pname make_tmp(char c, Ptype t, Ptable tbl) {
    ...
    if (tn && tn->tp)
        error('s', "defaultA too complicated");
    ...
    if (Cstmt) { //	make Cstmt into a block
        if (Cstmt->memtbl == 0)
            Cstmt->memtbl = new table(4, tbl, 0);
        tbl = Cstmt->memtbl;
...
Pexpr init_tmp(Pname tmp, Pexpr init, Ptable tbl) {
    ...
    if (ct) { // must initialize
        if (refd) {
            ...
            if (ct->tp->base == OVERLOAD)
                ct = Pgen(ct->tp)->fct_list->f; // first fct
            tbl = 0;
            // when tbl==0, call_ctor will not force declaration of ct
            if (ct->n_dcl_printed == 0)
                ct->dcl_print(0);
        }
    ...
    if (init->base == ZERO && tmp->tp->memptr())
        ...
        ass = new expr(ASSIGN, tmp, init); // no ctor: can assign
    ass->tp = tmp->tp;
    return ass;
}
    ...
    switch (nt->base) {
        case RPTR:
            if (nt->base == RPTR && Pptr(nt)->typ->check(at, COERCE) == 0)
                return 1;
            if (at == zero_type && Pptr(nt)->typ->is_ptr() == 0)
                return 0;
...
*/
{
    ...
    if (nt->check(at, 0)) {
        // handle T <-> const T
        if (const_problem && nt->base != PTR)
            return 1;

        // reject ptmfs of different classes
        if (rt && rt->memof || art && art->memof)
            return 0;
        ...
        if (rt && art)
            return 0; // ptrs do not match refs
        ...
        if (rt || (rt = nt->is_ptr())) {
            ...
            if (art) {
                if (art->typ->check(rt->typ, 0)) {
                    if (const_problem && Pbase(art->typ)->b_const != 1)
                        return 1;
                } else // T* -> T*const
                    return 1;
            }
...
*/
{
    ...
    if (c1 && !ref_cast && !zz) {
        ...
        if (c2 && same_class(Pclass(c2->tp), cl))
            return 1;
        ...
        if (f && f->base == FCT) {
            if (f->nargs == 1 || f->nargs > 1 && f->argtype->n_list->n_initializer) {
            ...
            for (gl = Pgen(f)->fct_list; gl; gl = gl->l) {
                ...
                if (ff->nargs == 1 || ff->nargs > 1 && ff->argtype->n_list->n_initializer) {
    ...
    if (val)
        return val;
    if (c1 && Pclass(c1->tp)->has_itor())
        return 0;
    if (t1->check(t2, COERCE))
        return 0;
    return 1;
}
...
*/
{
    ...
    for (e = arg, nn = f->argtype; e; e = e->e2, nn = nn->n_list) {
        if (nn == 0)
            return f->nargs_known == ELLIPSIS;
        Pexpr a = e->e1;
        Ptype at = a->tp;
        if (at->base == ANY)
            return NONE;
        if (exact1(nn, at)) {
            worst = min(worst, EXACT);
            continue;
        }
        if (exact2(nn, at)) {
            worst = min(worst, PROM);
            continue;
        }
        if (exact3(nn, at)) {
            worst = min(worst, STD);
...
*/
{
    ...
    switch (base) {
        ...
        case G_CALL:
            break;
        default:
            error('i', "call_fct(%k)", base);
    }

    // error('d',"call %d %k %n arg %d",this,e1->base,e1->base==NAME?e1:0,arg);
    if (t1 == 0)
        error('i', "call_fct(e1=%d,e1->tp=%t)", e1, t1);
    if (arg && arg->base != ELIST)
        error('i', "badAL%d%k", arg, arg->base);
...
lll:
    ...
    if (chk) {
        ...
        if (cl) {
            if (chk->n_oper == CTOR && chk->n_protect && cc->nof && cc->nof->n_oper == CTOR)
    ...
    if (fn)
        fn->use(); // a patch: ctors are sometimes not use()d
    ...
    for (nn = f->argtype, argno = 1; e || nn; nn = nn->n_list, e = etail->e2, argno++) {
        ...
        if (e) {
            ...
            if (nn) { /* type check */
                ...
                switch (t1->base) {
                    case RPTR: {
                        ...
                        if (pt->base != FCT || pt->check(a->tp, 0)) {
                            is_arg = 1;
                    ...
                    case COBJ:
                        if (a->base != G_CM || nn->tp->check(a->tp, ASSIGN))
                    ...
                    cbcb:
                        ...
                        if (a->base == G_CM) {
                            if (a->e1->base == DEREF)
                                a->e1 = a->e1->e2; // (*e1,e2) => (e1,e2)
                            if (a->e1->base == G_CALL && Pname(a->e1->fct_name)
                                && Pname(a->e1->fct_name)->n_oper == CTOR
                                && (a->e2->base == G_ADDROF || a->e2->base == ADDROF)) {
                                a = a->e1; // (ctor(&tmp),&tmp) => ctor(&tmp)
                                goto cccc;
                            } else if (a->e2->base == G_ADDROF && a->e2->e2->base == NAME) {
                            cccc:
                                if (t1->base == RPTR && Pptr(t1)->typ->tconst() == 0) {
                                    if (strict_opt)
                                        ...
                                        error('w',
                                              "temporary used for non-const%tA; no changes will be "
                                              "propagated to actual argument (anachronism)",
                                              nn->tp);
                                }
                            }
                        }
                    ...
                    case INT: {
                        ...
                        // no break
                    case LONG:
                        if (Pbase(t1)->b_unsigned && a->base == UMINUS && a->e2->base == ICON)
                ...
                if (e->e1->base == ILIST) {
                    ...
                    if (save_base == REF) {
                        ...
                        error(
                            ...
                            m->memof->string, m->memof->string, save_name);
                    }
                    if (mpt) {
            ...
            if (a->base == ILIST || ((a->base == CAST || a->base == G_CAST) && a->e1->base == ILIST)) {
                // memptr constant
                // f({1,2,f}) ==> memptr t; f((t={1,2,f},t))
                Pname temp = make_tmp('A', mptr_type, tbl);
                if (a->base != ILIST)
...
*/
{
    ...
    // error('d', "ref_init: nof: %n f_const: %d", cc?cc->nof:0,
    // cc?(cc->nof?Pfct(cc->nof->tp)->f_const:0):0);

    if (init->base == ILIST)
        error("IrL as RIr");

    if (init->base == NAME && Pname(init)->n_scope == ARG && init->tp->base == FLOAT)
    ...
    if (x == 0) { // CCC type is fine check for constness:
        if (init->tp->tconst() && vec_const == 0 && fct_const == 0) {
            // not ``it''
            if (init->base == ELIST)
                init = init->e1;
            if (px->typ->tconst() == 0)
                if (cc->nof && in_return)
                    error("cannot return a reference to a non-constO from const member function %n",
                          cc->nof);
                else
                    error("R to constO");
            px->base = RPTR;
        ...
        px->base = RPTR;
        if (init->lval(0)) {    // can pass the address
                                // // no temporary needed
            init->lval(ADDROF); // force output
                                // error('d',"px %t init %t ",px,init->tp);
            {
                ...
                if ((init->base == DEREF) && (init->e1 && (init->e1->base == NAME))) {
                    name_in_deref = Pname(init->e1);
                };

                if (name_in_deref && (name_in_deref->n_xref && name_in_deref->n_scope == ARG)) {
                    ...
                    if (init->e1->tp) {
                        ...
                        error("i", "No type for actual param %n", name_in_deref);
                    }
                } else {
    ...
    if (c1) { // assigning to a const X & is fine
        ...
        switch (init->base) {
            ...
            case NAME:
                ...
                break;
            default:
                refd = (init->e1 && init->e1->base == NAME && init->e1->tp->base != RPTR
                        && Pname(init->e1)->n_xref == 0)
                           ? 2
                           : 1;
                break;
        }
        ...
        switch (init->base) {
            ...
            case REF:
            case DOT:
                if ((it->tconst() == 0 || vec_const) && (fct_const == 0 || p1->is_ptr() == 0))
                    break;
            default:
                if (p1 && p1->b_const == 0) {
    ...
    if (p1->check(it, 0)) {
        ...
        if (p1_ptr && p1_ptr->typ->skiptypedefs()->base == FCT && it->is_or_pts_to(OVERLOAD)) {
            Pexpr op = ptof(Pfct(p1_ptr->typ->skiptypedefs()), init, tbl);
            if (op) {
                init = op;
                goto def;
            }
    ...
    switch (init->base) {
        ...
        case DOT: // init => &init
        {
            ...
            if ((init->base == NAME && Pname(init)->n_stclass == ENUM)
                || it_isconst && Pbase(p->typ) && !Pbase(p->typ)->tconst() && vec_const_save == 0
                       && fct_const_save == 0) {
                goto def;
            }
            init->lval(ADDROF);
        ...
        def : {
            ...
            refd = 1;
            Pexpr as;
            if (init->tp->memptr() || init->tp->base == FCT && Pfct(init->tp)->memof) {
...
*/
{
    ...
    if (c1) {
        ...
        if ((c1 != c2 && (c2 == 0 || same_class(cl, Pclass(c2->tp), 1) == 0))
            || (refd == 0 && cl->has_itor())) { // really ought to make a temp if refd,
...
Pexpr expr::docast(Ptable tbl) {
    ...
    switch (etp->base) {
        ...
        case COBJ: {
            ...
            if (x) {
                if (x != e1 && x->base == DEREF && tt->is_ref())
                    x = x->e1;
    ...
    switch (tt->base) {
        ...
        case PTR:
            if (tt->memptr()) {
                if (etp->memptr() == 0 && (etp->base != FCT || Pfct(etp)->memof == 0)) {
                    if (etp->base == ZTYPE) {
        ...
        case RPTR: // (x&)e: pretend e is an x
        {
            ...
            Ptype cr = tt;
            do {
                if (er = er->is_ptr_or_ref())
                    er = Pptr(er)->typ;
                if (cr = cr->is_ptr_or_ref())
                    cr = Pptr(cr)->typ;
            } while (er && cr);
            ...
            if (e1->base == G_CM || e1->base == CALL || e1->base == G_CALL || e1->lval(0))
                ;
    ...
    if (etp->memptr()) {
        ...
        if (Pptr(tt)->memof == 0 && b == 0) {
            Pexpr y;
            if ((e1->base == G_CAST || e1->base == CAST) && e1->e1->base == ILIST) {
                e1 = e1->e1;
                y = e1->e2;
            } else {
                y = new mdot("f", e1);
    ...
    if (e1 && e1->e2 == 0) { // single argument
        ...
        if (acn && cl->has_itor() == 0) {
            ...
            if (same_class(acl, cl) || hb) {
                vcllist->clear();
                vcllist = 0;
                if (1 < is_unique_base(acl, cl->string, 0))
                    error("ambiguous assignment to base %t", cl);
                Pexpr ee = e1->e1;
                if (ee->base == ELIST)
                    ee = ee->e1; // ???
                if (hb) {        // ee => *(tp2*)&ee
...
mk_ctor_call: // beats duplicating the code
    ...
    if (e2 == 0) { // x(a) => x temp; (temp.x(a),temp)
        if (e1 && e1->e1 && !e1->e2) {
            char *s = e1->e1->string;
            if (s && s[0] == '_' && s[1] == '_' && s[2] == 'V' && e1->e1->tp && tp2
                && !e1->e1->tp->check(tp2, 0))
                return e1;
        }
        no_sti = 1;
        Ntmp = 0;
...
Pname gen::exactMatch(Pexpr arg, int constObj)
/*
...
Pname gen::oneArgMatch(Pexpr aarg, int constObj)
/*
...
*/
{
    ...
    for (gl = fct_list; gl; gl = gl->l) {
        ...
        if (constObj && fn->n_oper != CTOR && !ft->f_const && !ft->f_static) {
            non_const++;
            continue;
        }
...
Pname gen::multArgMatch(Pexpr arg, int constObj)
/*
                ...
                if (miFlag == 1 && numFunc > 2) {
                    ...
                    for (int K = 0; K < numFunc; K++) {
                        ...
                        for (int I = 0; I < numargs; I++) {
                            if (bestOfPair(intFun[I][sigbit], intFun[I][K], targ->e1->tp)) {
                                gotit = 1;
                                break;
                            }
                            targ = targ->e2;
                        ...
                        if (!gotit) {
                            ...
                            Bits temp = bestFuncs;
                            bestFuncs.set(K);
                            if (breakTie(funVec, temp, arg, constObj) != funVec[sigbit]) {
                                fmError(1, funVec, arg, constObj);
                                break;
                            }
                        }
...
Bits best_const(const Block(Pname) & CONV, int nfound, Ptype at) {
    ...
    while (tempBits.count()) {
        ...
        for (int k = nfound - 1; k > tempPtr; k--) {
            ...
            if (p1 && p2) {
                if (at->check(t1, OVERLOAD) == 0 && at->check(t2, OVERLOAD))
                    result.reset(k);
                else if (at->check(t1, OVERLOAD) && at->check(t2, OVERLOAD) == 0) {
                    result.reset(tempPtr);
                    break;
                }
            } else {
                ...
                if (p1 && p2) {
                    if (at->check(p1->typ, OVERLOAD) == 0 && at->check(p2->typ, OVERLOAD))
                        result.reset(k);
                    else if (at->check(p1->typ, OVERLOAD) && at->check(p2->typ, OVERLOAD) == 0) {
                        result.reset(tempPtr);
                        break;
                    }
                }
...
void fmError(int errorKind, const Block(Pname) & FV, Pexpr arg, bit co) {
    Pname fn = FV[0]->tp->base == OVERLOAD ? Pgen(FV[0]->tp)->fct_list->f : FV[0];
    ...
    if (FV[0]->tp->base == OVERLOAD) {
        ...
        for (Plist gl = Pgen(FV[0]->tp)->fct_list; gl; gl = gl->l) {
            if (gl->f->is_template_fct()) {
                ...
                error('C', "	%a;\n", gl->f);
            if (co && !Pfct(gl->f->tp)->f_const)
                no_const++;
            num_all++;
        }
        if (num_templ) {
            error('C', "	%d template version%s;\n", num_templ, num_templ == 1 ? "" : "s");
        }
        if (no_const == num_all)
