    ...
    */
    ...
    switch (b1) {
        ...
        case FCT: {
            ...
            if (f1->returns->check(f2->returns, rec_oper0, level + 1)) {
                if (vv && cnst1 == cnst2) {
                    ...
                    Ptype t1 = f1->returns;
                    Ptype t2 = f2->returns;
                    if ((t1->is_ptr() && t2->is_ptr()) || (t1->is_ref() && t2->is_ref())) {
                        t1 = Pptr(t1->is_ptr_or_ref())->typ;
                        t2 = Pptr(t2->is_ptr_or_ref())->typ;
                        if (!t1->is_ptr_or_ref() && !t2->is_ptr_or_ref()) {
                            t1 = t1->skiptypedefs();
                            t2 = t2->skiptypedefs();
        ...
        case COBJ: {
            ...
            if (same_class(Pclass(b1 == COBJ ? n1->tp : t1), Pclass(b1 == COBJ ? n2->tp : t2),
                           access || is_arg)
                || rec_oper == PT_OVERLOAD
                       && fm_same_class(Pclass(b1 == COBJ ? n1->tp : t1),
                                        Pclass(b1 == COBJ ? n2->tp : t2)))
                goto const_check;
            ...
            switch (oper) {
                ...
                pt_hack:
                    ...
                    if (level <= 1
                        && ((Pclass(b1 == COBJ ? n2->tp : t2))
                                ->is_base(b1 == COBJ ? n1->string : Pclass(t1)->string))) {
                        if (ppbase != PUBLIC) {
                            const_problem = 0;
