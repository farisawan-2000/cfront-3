...
*/
{
    ...
    while (n = syn()) {
        ...
        DB(if (Rdebug >= 1)
               error('d', "run: syn():%n%t base%k ll %d", n, n->tp, n->base, n->lex_level););
        ...
        for (Pname nx, nn = n; nn; nn = nx) {
            ...
            nn->n_list = 0;
            DB(if (Rdebug >= 1)
                   error('d', "run dcl:%n%t base%k ll %d", nn, nn->tp, nn->base, nn->lex_level);
               if (Rdebug >= 2) display_expr(nn, "nn"););

            if ((rr = nn->dcl(gtbl, EXTERN)) == 0 || nn->base == 0 || (error_count > oerror_count))
                continue;
            DB(if (Rdebug >= 1) if (rr)
                   error('d', "run rr:%n%t base%k ll %d", rr, rr->tp, rr->base, rr->lex_level);
               else error('d', "run rr: 0"); if (Rdebug >= 2) display_expr(rr, "rr"););

            DB(if (Rdebug >= 1)
                   error('d', "run simpl:%n%t base%k ll %d", nn, nn->tp, nn->base, nn->lex_level););
            nn->simpl();
            ...
            if (nn->base != TNAME) {
                ...
                switch (t->base) {
                    ...
                    case ENUM:
                        ...
                        break;
                    case FCT:
                        DB(if (Rdebug >= 1) error('d', " -- fct%n inline %d body %d", nn,
                                                  Pfct(t)->f_inline, Pfct(t)->body););
                        if (Pfct(t)->body == 0
                            || (debug_opt == 0 && Pfct(t)->f_inline
                                && (n->n_table != gtbl || strcmp(nn->string, "main")))
                            || Pfct(t)->f_imeasure && Pfct(t)->f_inline == 0)
                            break;
                        DB(if (Rdebug >= 1) error('d', "rr%n inline %d body %d", rr,
                                                  Pfct(rr->tp)->f_inline, Pfct(rr->tp)->body););
                        rr->dcl_print(0);
                        break;
                    ...
                    default: {
                        ...
                        for (Ptype tx = t;;) {
                            switch (tx->base) {
                                ...
                                case CLASS: {
                                    Pclass cl = Pclass(tx);
                                    if ((cl->defined & (DEFINED | SIMPLIFIED)) && cl->c_body == 1)
                                        cl->dcl_print(0);
                                    break;
                                }
                            }
            ...
            switch (nn->tp->base) { // clean up
                ...
                case FCT: {
                    if (nn->n_oper == CTOR)
                        break; // at least for now, need the f_this
                    Pfct f = Pfct(nn->tp);
                    DB(if (Rdebug >= 1) error('d', "function nn:%n local_class: %d %d", nn, local_class,
                                              f->local_class););
    ...
    for (Pname m = gtbl->get_mem(i); m; NEXT_NAME(gtbl, m, i)) {
        if (m->base == TNAME || m->n_sto == EXTERN || m->n_stclass == ENUM)
            continue;
        ...
        if (m->n_addr_taken == 0 && m->n_used == 0 && m->n_sto == STATIC) { // (static or anon?)
            for (;;) {
    ...
    if (strict_opt == 0) { // define static members
        ...
        for (Plist l = stat_mem_list; l; l = l->l) {
            ...
            if (n->n_initializer == 0 && n->n_evaluated == 0) {
                ...
                if (cn
                    && (Pclass(cn->tp)->has_ctor() || t->is_ref()
    ...
    for (Pname nm = ptbl->get_mem(i); nm; NEXT_NAME(ptbl, nm, i)) {
        DB(if (Rdebug >= 1)
               error('d', "ptbl anme %s string2 %s key:%k", nm->string == 0 ? "???" : nm->string2,
                     nm->string2 == 0 ? "???" : nm->string2, nm->n_key););
...
void process_debug_flags(char *p) {
    ...
    while (c = *p++) {
        switch (c) {
            ...
            case 'Y':
                ...
                break;
            default:
                error('w', "unknown debugging flag '%c'", c);
        }
    }
}
#endif /*DBG*/
