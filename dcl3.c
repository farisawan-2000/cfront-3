...
Pname gen::match(Pname nx, Pfct f, bit warn) {
    ...
    if (fx->nargs != f->nargs && fx->nargs_known == 1 && f->nargs_known == 1)
    ...
    for (ax = fx->argtype, a = f->argtype; a && ax; ax = ax->n_list, a = a->n_list) {
        ...
        */
        ...
        if (const_problem) { // differ only in X vs const X
            if (at->is_ptr() && atp->is_ptr() || at->is_ref() && atp->is_ref()) {
        ...
        if (atp->base == RPTR) { // differ only by X vs X& ?
            ...
            if (Pptr(atp)->typ->check(at, 255) == 0
                || const_problem && (at->is_ptr() == 0 || at->tconst() != Pptr(atp)->typ->tconst())) {
                op++;
                continue;
            }
        }
        ...
        if (at->base == RPTR) { // differ only by X& vs X ?
            // require strict-any-check for templates
            if (Pptr(at)->typ->check(atp, 255) == 0
                || const_problem && (atp->is_ptr() == 0 || atp->tconst() != Pptr(at)->typ->tconst())) {
                op++;
                continue;
            }
            break;
...
static int pts_to_templ(Ptype t) {
...
static void grab_braces(Pexpr *base, int lst) {
    int flag = 0;
...
zzz:
    switch (t->base) {
                ...
                vtz:
                    ...
                    switch (vt->base) {
                        ...
                        default: {
                            ...
                            Pptr p;
                            if (vt->check(e->tp, 0) && (p = vt->is_ptr()) && Ptype(p) != zero_type
                                && p->typ != char_type) {
                                Pexpr te = e;
                                Ptype t = p->typ;
                                while (t->base == TYPE)
        ...
        ccc:
            ...
            if (cl->baselist) {
                if (cl->baselist->next)
                    error("IrL forO ofC with multipleBCs");
                ...
                switch (mt->base) {
                    ...
                    case CLASS:
                        if (mt->base == CLASS) {
                            Pclass cl = (Pclass) mt;
                            if (!cl->is_simple())
                                error("cannotI%nWIrL because class %s has private or protected members",
                                      nn, cl->string);
                        }
                    case ENUM:
                        continue;
            ...
            dd:
                ...
                if ((MP = mt->memptr()) && e == Pexpr(1) && last_il->tp->base == PTR) {
                    if (i == 1)
                        lst = 0;
                } else
                    e = next_elem();
                ...
                if (mt->base == FIELD && m->string[0] == '_' && m->string[1] == '_'
                ...
                switch (mt->base) {
                    ...
                    default: {
                        ...
                        Pptr p;
                        if (mt->check(e->tp, 0) && (p = mt->is_ptr()) && Ptype(p) != zero_type
                            && p->typ != char_type)
        ...
        default:
            ...
            Pptr p;
            if (t->check(e->tp, 0) && (p = t->is_ptr()) && Ptype(p) != zero_type && p->typ != char_type)
                *curr_e = new cast(t, e);
            if (lst && (e = next_elem()))
...
Pname dclass(Pname n, Ptable tbl) {
    ...
    if (Pclass(bn->tp)->class_base == INSTANTIATED)
        nx->where = n->where;
    bn->where = nx->where;
...
static int illegal_local_ref(Pname nn) {
    switch (nn->n_scope) {
        default:
            return 0;
        case ARG:
...
static int size_t_check(Ptype tp)
/* returns 0 if the given type is compatible with size_t_type,
   returns 1 otherwise */
{
    if (tp->check(size_t_type, 0) == 0)
        return 0;
    int size_t_size = size_t_type->tsizeof();
    if (size_t_size == ulong_type->tsizeof() && tp->check(ulong_type, 0) == 0)
        return 0;
    if (size_t_size == ushort_type->tsizeof() && tp->check(ushort_type, 0) == 0)
        return 0;
    return 1;
}

static void check_for_local(Pexpr ee) {
    ...
    switch (ee->base) {
        case NAME: {
            Pname nn = Pname(ee);
            if ((nn->n_scope == FCT || nn->n_scope == ARG) && illegal_local_ref(nn)
                && is_probably_temp(nn->string) == 0 && n[0] != nn && n[1] != nn) {
    ...
    for (Pname a = f->argtype; a; a = a->n_list) {
        ...
        if (
            ...
            if (cln) {
                ...
                Pptr p = a->tp->is_ptr();
                if (perf_opt && (init->base == NEW || init->base == GNEW))
                    New_in_arg_list = 1;
                init = init->typ(tbl);
                if (p)
...
Pname name::dofct(Ptable tbl, TOK scope) {
    ...
    DB(if (Ddebug >= 1) {
        ...
        error('d', &where, "  n_oper%k init %d f_virtual %d f_inline %d", n_oper, n_initializer,
              f->f_virtual, f->f_inline);
    });
    ...
    if (n_qualifier) { // qualified name: c::f() checked above
        ...
        if (n_sto && n_sto != FRIEND // friend X::f();
    ...
    switch (n_oper) {
        ...
        case REF:
            ...
            if (f->returns->is_ptr() == 0) {
                ...
                if (cn == 0 || Pclass(cn->tp)->has_oper(REF) == 0) {
                    if (cn && class_name && strcmp(cn->string, class_name->string) == 0)
        ...
        case DELETE: // void operator delete(void*) or
            ...
            if (class_name) {
                ...
                if (in_class_dcl && cl->memtbl->look("__dl", 0))
                    ...
                    switch (t->base) {
                        case TYPE:
                            if (warning_opt && Pbase(t)->b_const == 0)
                                error('w', "assignment operators should take a const %t& not a %t&", t,
                                      t);
                            t = Pbase(t)->b_name->tp;
                            goto rx1;
                        case COBJ:
                        ...
                        case FCT:
                            if (tp->check(nx->tp, 0) && n_sto != EXTERN)
                                ...
                                if (Pfct(nx->tp)->f_linkage != Pfct(tp)->f_linkage)
                                    error('w',
                                          "%n of type %t has been locally re-declared with different "
                                          "linkage",
                                          this, tp);
                            }
                            break;
                        case OVERLOAD: {
                            ...
                            if (ny == 0)
                                ...
                                if (Pfct(ny->tp)->f_linkage != Pfct(tp)->f_linkage)
                                    error('w',
                                          "overloadedF%n of type %t has been locally re-declared with "
                                          "different linkage",
                                          this, tp);
                            }
                        } break;
    ...
    if (Nold && f->fct_base != INSTANTIATED) {
        ...
        if (nf->base == ANY || f->base == ANY)
            ...
            if (f->body && nf->body) {
                ...
                if (class_name == 0 || Pclass(class_name->tp)->class_base != INSTANTIATED
                    || nn->n_redefined == 0) {
            ...
            {
                TOK t = (nf->fct_base == FCT_TEMPLATE || f->fct_base == FCT_TEMPLATE) ? PT_OVERLOAD
                                                                                      : OVERLOAD;
        ...
        bdbd:
            // error('d',"nn%n init: %d f_virt: %d f->body: %d",
            // nn,nn->n_initializer,nf->f_virtual,f->body);
            ...
            if (f->f_inline) {
                if (nf->f_inline == 0) {
                    if (nn->n_used && nn->n_sto != STATIC)
                        error("%n declared with external linkage and called before defined as inline",
                              nn);
                    else if (nn->n_used) {
        ...
        stst2:
            ...
            if (n_sto == STATIC && nn->n_sto == EXTERN
                && (strcmp(string, "__nw") == 0 || strcmp(string, "__dl") == 0))
                nn->n_sto = STATIC;
    ...
    thth:
        ...
        if (class_name && f->f_static == 0 // no ``this'' in static members
        ...
        switch (n_oper) {
                ...
                switch (f->nargs) { // check for X(X) and X(X&)
                    ...
                    clll:
                        switch (t->base) {
                            ...
                            case COBJ: /* X(X) ? */
                                if (class_name == Pbase(t)->b_name) {
                                    error("badK %s(%s) use %s(%s&)", class_name->string,
                                          class_name->string, class_name->string, class_name->string);
            ...
            default: {
                ...
                switch (n_oper) {
                    ...
                    case ASRS:
                    case CM:
                        if ((!class_name && (memberonly || f->nargs == 2))
                            || (class_name && f->nargs == 1))
                            ;
                        else
                    ...
                    case INCR:
                    case DECR:
                        if ((!class_name && (f->nargs == 1 || f->nargs == 2))
                            || (class_name && f->nargs <= 1))
                            ;
                        else
