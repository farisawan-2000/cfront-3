            ...
            tx:
                switch (mt->base) {
                    ...
                    case COBJ: {
                        ...
                        if (slow == 0 // if slow, previous members have already been copied
                    ...
                    default:
                        if (slow) {
                            ...
                            ee->n_initializer =
                                new ref(DOT, new name(arg->string), new name(m->string));
                            if (e)
                                ee->n_list = e;
                            e = ee;
                        }
...
/*	is the operator an assignment operator? */
{
    ...
    if (an && same_class(Pclass(an->tp), tcl))
        return t;
    return 0;
}
...
*/
{
    ...
    if (slow) {
        for (Pbcl b = cl->baselist; b; b = b->next) {
            ...
            Ptype pt = bcl->this_type;
            switch (b->base) {
                case NAME: { // generate: *(bcl*)this = *(bcl*)&arg;
                             // error('d',"base %t",bcl);
            ...
            switch (mt->base) {
                ...
                case COBJ:
                    // error('d',"cobj %n %d %d",m,slow,Pclass(Pbase(mt)->b_name->tp)->c_xref);
                    {
                        ...
                        if (slow == 0
                            && Pclass(Pbase(mt)->b_name->tp)->c_xref
                                   & (C_VBASE | C_ASS)) {    // must use its assignment operation
...
void prnt_all_in_scope(Pclass cl) {
    ...
    for (Pname nn = cl->memtbl->get_mem(i = 1); nn; NEXT_NAME(cl->memtbl, nn, i)) {
        if (nn->base == TNAME)
            continue;
        if (((nn->base) && nn->base == NAME) && (nn->n_anon == 0) && (nn->n_stclass == STATIC)
            && ((nn->tp)
                && (nn->tp->base != FCT && nn->tp->base != OVERLOAD && nn->tp->base != CLASS
                    && nn->tp->base != ENUM))) {
            nn->dcl_print(0);
        }
    }
...
void classdef::dcl(Pname cname, Ptable tbl) {
    ...
    if (baselist) {
        ...
        */
        ...
        for (Pbcl lx, l = baselist; l; l = lx) { // remove bad bases
            ...
            if (warning_opt && (l->base == VIRTUAL || l != baselist) && cl->has_dtor()
                && cl->has_dtor()->tp && !Pfct(cl->has_dtor()->tp)->f_virtual)
        ...
        */
        ...
        if (scope == 0) {
            ...
            if (nn) {
                ...
                if (Pclass(nn->tp)->csu == CLASS && strcmp(this->string, nn->string))
                    st = 0;
            }
        }
        ...
        if (in_union == ANON) {
            if (p->tp->base == CLASS && Pclass(p->tp)->csu == ANON) {
                ...
                while (pu) {
                    // error('d',&p->where,"pu%n base%k tp%k: ll
                    // %d",pu,pu->base,pu->tp->base,pu->lex_level);
                    Ptype tx = 0;
                    if (pu->base == TNAME)
                        tx = pu->tpdef;
            ...
            if (p->base != TNAME && p->tp->base != CLASS && p->tp->base != ENUM)
                --p->lex_level;
        } // if this is anon
          // error('d',&p->where," -- p%n base%k tp%k: ll %d",p,p->base,p->tp->base,p->lex_level);
        ...
        if (p->tp->base == FCT) {
            ...
            if (m->n_initializer) {
                ...
                if (m->n_oper == DTOR)
                    error('w',
                          "please provide an out-of-line definition: %n {}; which is needed by derived "
                          "classes",
                          m);
            }
            if (ff == 0)
            ...
            // error('d',"lex_level %d p %n lex %d nested_scope: %k",lex_level,p,p->lex_level,
            // nested_scope);

            TOK is_friend = p->n_sto;
            if (protect && p->tp->base == COBJ && Pclass(Pbase(p->tp)->b_name->tp)->csu == ANON)
                p->n_protect = protect;
    ...
    for (Pname m = memtbl->get_mem(i = 1); m; NEXT_NAME(memtbl, m, i)) {
        ...
        */
        ...
        switch (t->base) {
            default:
                if (ct == 0 && m->n_stclass != ENUM && m->n_stclass != STATIC) {
                    if (t->is_ref())
                        error("R%n inC%tWoutK", m, this);
                    if (t->tconst() && vec_const == 0 && m->n_evaluated == 0)
    ...
    if (pt_opt && !nested_sig && !local_sig && !is_probably_temp(string) && !tempdcl)
        if (csu == UNION)
...
#define CHECKU(t)                                                                                      \
    { ok = (unsigned t) ii <= largest_int; }
#define CHECKS(t)                                                                                      \
    { ok = (t) ii >= smallest_int && (t) ii <= largest_int; }
                    case CHAR:
                        if (u)
                            CHECKU(char)
                        else
