...
void Eprint(Pexpr e) {
    ...
    switch (e->base) {
        case DOT:
        case REF:
            if (Pref(e)->mem && Pref(e)->mem->tp
                && Pref(e)->mem->tp->base == FCT) { // suppress ``this'' in ``this->f''
                Pref(e)->mem->print();
                break;
            }
        case NAME:
...
void expr::print() {
    if (this == 0)
        error('i', "0->E::print()");
    if ((this == e1 || this == e2) && base != NAME)
        error('i', "(%p%k)->E::print(%p %p)", this, base, e1, e2);
    ...
    switch (base) {
        ...
        case ANAME:
            ...
            aok:
                if (n = il->i_args[argno].local) {
                    ...
                    if (ee->tp == 0 || (t != ee->tp && t->check(ee->tp, 0) && t->is_cl_obj() == 0)) {
                        putstring("((");
                        bit oc = Cast;
                        Cast = 1;
                        t->print();
        ...
        case SIZEOF:
            puttok(SIZEOF);
            if (e1 && e1 != dummy && e1->base != ILIST
                && ((e1->base != CAST && e1->base != G_CAST) || e1->e1->base != ILIST)) {
                eprint(e1);
            } else if (tp2) {
                putch('(');
                if (tp2->base == CLASS) {
        ...
        case CALL: {
            ...
            if (fn) {
                ...
                if (f) { // pointer to fct
                    ...
                    if (exex->base == DEREF) {
                        exex = exex->e1;
                        while (exex->base == CAST || exex->base == G_CAST)
                            exex = exex->e1;
                        if (exex->base == MDOT)
                            m_ptr = 1;
                    }
            ...
            if (e2) {
                if (at) {
                    ...
                    while (at) {
                        ...
                        if (t != ex->tp && ex->tp && t->check(ex->tp, 0) && t->is_cl_obj() == 0
                            && eobj == 0 && m_ptr == 0 && (t->is_ptr() == 0 || Mptr == 0)) {
                            putch('(');
                            bit oc = Cast;
                            Cast = 1;
                            t->print();
...
#endif
                                if (ex->tp->is_cl_obj()
                                    && ((ex->base != NAME && ex->base != ANAME)
                                        || Pname(ex)->n_xref == 0)
                                    && // beware of reference arguments
                                    (t->is_ptr() || t->is_ref())) {
        ...
        case ASSIGN:
            if (e1->base == ANAME
                && Pname(e1)->n_assigned_to == FUDGE111) { // suppress assignment to "this"
        ...
        bkk : {
            ...
            if (e1->tp && e1->tp != e2->tp && e2->base != ZERO) { // cast, but beware of int!=long etc.
                ...
                switch (t1->base) {
                    ...
                    case VEC: {
                        ...
                        if (e2->tp == 0
                            || (!ansi_opt && Pptr(t1)->typ && Pptr(t1)->typ->skiptypedefs()->base == VEC
                                && e2->base != G_CAST && e2->base != CAST)
                            || (Pptr(t1)->typ != Pptr(t2)->typ && t1->check(t2, 0)
                                && (t1->memptr() == 0 || t2->memptr() == 0 || t1->check(t2, COERCE)))) {
                            putch('(');
                            bit oc = Cast;
                            Cast = 1;
                            e1->tp->print();
        ...
        case QUEST: { // look for (&a == 0) etc.
            ...
            if (Neval == 0)
                ...
                putch('?');
                if (!ansi_opt) {
                    if ((e1->base == CAST || e1->base == G_CAST) && e1->tp
                        && !e1->tp->check(void_type, 0)) {
                        e1 = new expr(G_CM, e1->e1, zero);
                        e1->tp = zero_type;
                    }
                    if ((e2->base == CAST || e2->base == G_CAST) && e2->tp
                        && !e2->tp->check(void_type, 0)) {
        ...
        case G_ADDROF: {
            switch (e2->base) { // & *e1 or &e1[e2]
                ...
                case NAME: {
                    ...
                    if ((e2->tp) && (e2->tp->is_cl_obj()) && n->n_xref) {
                        eprint(e2);
                        return;
                    }
                    if (!ansi_opt && (e2->tp && e2->tp->skiptypedefs()->base == VEC)) {
...
void stmt::print() {
    ...
    if (memtbl && base != BLOCK) { /* also print declarations of temporaries */
        ...
        for (Pname n = tbl->get_mem(i = 1); n; NEXT_NAME(tbl, n, i)) {
            ...
            Pname cn;
            if (bl && (cn = n->tp->is_cl_obj()) && Pclass(cn->tp)->has_dtor())
                bl = 0;
        }
        ...
        case BLOCK:
            ...
            if (memtbl && own_tbl) {
                ...
                for (n = memtbl->get_mem(i = 1); n; NEXT_NAME(memtbl, n, i)) {
                    DB(if (Pdebug >= 1) error('d', &n->where, "print local%n base%k tp%t scope%k", n,
                                              n->base, n->tp, n->n_scope));
                    if (n->tp && n->n_anon == 0 && n->tp != any_type)
                        switch (n->n_scope) {
                            case ARGT:
                            case ARG:
                                break;
                            default:
                                ...
                                if (n->base == TNAME && n->tpdef && n->tpdef->nested_sig)
