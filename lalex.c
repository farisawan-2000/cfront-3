...
gettok:
        ...
        if (tk == TNAME) {
            ...
            // Have an ID. Check last token to be sure.
                ...
                if (n == 0 // no hidden type name ...
                    || (n->n_template_arg == template_type_formal) || tagdef) {
                    // error('d', "ccl%t ll %d bl %d in_class_decl %d", ccl, ccl?ccl->lex_level:0,
                    // bl_level, in_class_decl);
                    // XXXXX currently enter all unqualified
                    // XXXXX    friends in Gtbl
                    // XXXXX Should eventually enter in enclosing
                    // XXXXX    scope, however back end currently
                ...
                switch (tk2) {
                    ...
                    case ENUM: {
                        ...
                        if (n) {
                            ...
                            switch (n->tp->base) {
                                default:
                                    error("%s%s:Tdef %sis %sin this scope", s, x, s,
                                          n->n_key == HIDDEN ? "hidden" : "undefined");
                                    break;
                                case COBJ:
                                    error("%sis %sin this scope: useC %s%s", s,
                                          n->n_key == HIDDEN ? "hidden" : "undefined", s, x);
                                    break;
                                case EOBJ:
                                    error("%sis %sin this scope: use enum %s%s", s,
                                          n->n_key == HIDDEN ? "hidden" : "undefined", s, x);
                                    break;
                            }
                            tk = TNAME;
                        ...
                        // SYM -- else nested lookup removed
                            ...
                            if (tk2 == ID) {
                                ...
                                if (lasttk == TSCOPE && lastval.pn->base == TNAME) {
                                    ...
                                    else if (lastval.pn->tp->base == COBJ
    ...
    if (tk == TNAME || (tk == TYPE && latype(yylval.t)) || tk == REF || tk == DOT || tk == GOTO
        || tk == MEMPTR)
...
ret:
    ...
    lasttk = tk;
    lastval = yylval;
    LDB(1, showQ("after"); fprintf(stderr, "lalex returning ");
        printok2(tk == ID ? NAME : tk, yylval, curloc););
    // error('d',"returning tk: %k dtor_seen: %n", tk,dtor_seen );
    return tk;
}

extern void la_backup(TOK t, YYSTYPE r)
/*
...
static TOK ptname_tscope(toknode *lt) {
    int nest = 0; // the LT has been fetched
    if (lt->tok != LT)
    ...
    TOK tk;
    for (toknode *t = lt;; t = get_next_token(t)) {
        // error('d',"ptname_tscope: t: %k t->next: %k",t->tok,
        // t->next?t->next->tok:(add_tokens(),t->next->tok));
        switch (t->tok) {
            case LT:
                ++nest;
                continue;
...
//    have an effect
#define RESET_SCOPE(s)                                                                                 \
    if (s > 0)                                                                                         \
        do                                                                                             \
            UNSET_SCOPE();                                                                             \
        while (--s);

static int la_decl(int arg_decl)
/*
        handles ambiguities
                type (*x) ()
                type (*x) []
...
*/
{
    ...
    if (tk2 != LP)
        error('d', &latok->place, "la_decl(): latok ==%k -- '(' expected", tk2);
        ...
        switch (tk) {
            ...
            case TSCOPE:
                if ((tk2 == ID || tk2 == OPERATOR) && !arg_decl) {
                    // T ( * C :: id ...
                    if (latok->last->last->tok == TNAME && SET_SCOPE(latok->last->last->retval.pn))
                        ++scopesets;
                    continue;
                } else {
                    // error
    ...
    for (;;) {
        ...
        switch (tk) {
            case LP:
                ...
                if (tk2 == RP || tk2 == ENUM || tk2 == AGGR || tk2 == ELLIPSIS
                    || tk2 == TYPE && la_sctype(tk2)) {
                    // T ( * id ()
                    // T ( * id ) ()
                    RESET_SCOPE(scopesets);
                    return 1;
            ...
            case RP:
                ...
                } else if (tk2 == RP && (bl_level - in_class_decl == 0)) {
                    RESET_SCOPE(scopesets);
                    return !arg_decl;
                } else {
                    // T ( * id ) ...
