    ...
    switch (which_union) {
        ...
        case 3: /* tp2, fct_name, cond, mem, as_type, n_table, il, query_this */
            switch (base) {
                ...
                case NAME:
                case TNAME:
                    if (Pname(this)->n_oper == TYPE && (strcmp(Pname(this)->string, "_type") == 0)) {
...
node_class classify_node(Pnode node, int &error) {
    ...
    if (!map_initialized) {
        map_initialized = 1;
        for (ncx = 0; ncx < sizeof(token_classes) / sizeof(token_class); ncx++)
            token_to_class_map[token_classes[ncx].token] = token_classes[ncx].nclass;
    }
...
node_class classify_node(Pnode node) {
    ...
    if (err)
        error('i', "failed to classify node.");
    return nclass;
}
