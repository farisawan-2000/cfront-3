    ...
    while (s) {
        switch (s->base) {
            case RETURN: {
                ...
                if (!te || strcmp(te->string, v->string)) {
                    return s;
                }
                if (te == v)
                    return 0;
...
Pexpr check_cond(Pexpr e, TOK b, Ptable tbl) {
    ...
    if (cn = e->tp->is_cl_obj()) {
        ...
        switch (nfound) {
            ...
            case 2:
                error("two conversions for%nO in%kE: %n and %n", cn, b, found, other);
                break;
            default:
        ...
        switch (ss->base) {
                    ...
                    case VALUE:
                        ...
                        if (ss->e->tp->base != VOID) {
                            if (ss->e->base == DEREF)
                                ...
                                if (ss->e->base == NAME && ss->e->tp->base == FCT) {
            ...
            case RETURN: {
                ...
                if (v != dummy) {
                    ...
                    if (fn->n_oper == CTOR || fn->n_oper == DTOR || rt->base == VOID) {
                        ...
                        switch (rt->base) {
                            case RPTR:
                                switch (v->base) {
                                    case NAME:
                                        if (Pname(v)->n_scope == FCT || Pname(v)->n_scope == ARG)
                                ...
                                in_return = 0;
                                if (v->base == G_CM && v->e2->base == G_ADDROF
                                    && v->e2->e2->base == NAME && is_probably_temp(v->e2->e2->string))
                                    error('w', "R to temporary returned (return value is not lvalue or "
                                               "of wrongT)");
                            case ANY:
                                break;
                            case COBJ:
                                if (v->base == DEREF) {
                                    Pexpr v1 = v->e1;
                                    if (v1->base == CAST || v1->base == G_CAST) {
                                        Pexpr v2 = v1->e1;
                                        if (v2->base == G_CM) {
                                            // *(T)(e1,e2) => (e1,*(T)e2)
                                            Pexpr v3 = v2->e2;
                            ...
                            case PTR: {
                                ...
                                if (v->base == ADDROF && v->e2->base == NAME
                            ...
                            case LONG:
                            case SHORT:
                                if (Pbase(rt)->b_unsigned && v->base == UMINUS && v->e2->base == ICON)
            ...
            case SWITCH: {
                ...
                if (ne) { // see if the number of cases is "close to"
                    ...
                    if (i && i != ne) {
                        ...
                        ee:
                            if (ss->has_default == 0)
                                error('w', "switch (%t)W %d cases (%d enumerators)", ss->e->tp, i, ne);
                        } else {
                            switch (ne - i) {
                                case 1:
                                    if (3 < ne)
                        ...
                        default:
                            ...
                            //	must not change
                            {
                                ...
                                *ss = *fi;
                                ...
                                ss->dcl(1);
                                tmp = ss->s_list;
                                if (ss->base == DCL && tmp->base == FOR // sanity check
            ...
            case DCL: /* declaration after statement */
            {
                ...
                for (Pname nn = tail; nn; nn = nn->n_list) {
                    ...
                    if (non_trivial >= 2)
                        continue;
                    if (nn->n_sto == STATIC && nn->tp->base != FCT || nn->tp->is_ref()
                ...
                // error('d',"dcl stmt : d %n  non_trivial %d  curr own_tbl %d  inline_restr
                // 0%o",ss->d,non_trivial,curr_block->own_tbl,inline_restr);
                if (non_trivial == 3 || non_trivial == 2 // must
                    || non_trivial == 1                  // might
...

                    {
                        ...
                        for (nn = ss->d; nn; nn = nn->n_list) {
                            Pname n;
                            // error('d',"checking %n  lex_level: %d  n: %n  n->lex_level:
                            // %d",nn,nn->lex_level,n,n?n->lex_level:0); error('d',"   own_tbl: %d
                            // curr_block: %d  n->n_table:
                            // %d",curr_block->own_tbl,curr_block,n->n_table); error('d',"   real_block:
                            // %d  n's real_block:
                            // %d",curr_block->memtbl->real_block,n->n_table->real_block);
...
*/
{
    ...
    if (f->f_result && s && f->nrv == 0
        ...
        if (tmp->base == RETURN) {
            ...
            Pname td;
            if (tt && (td = is_decl(curr_block, tt)) && td->n_sto != STATIC
        ...
        if (f->nrv) {
            if (find_return(s, f->nrv))
                f->nrv = 0;
        }
    } else if (f->nrv)
        re_decl = is_decl(curr_block, f->nrv);
    ...
    if (d) {
        ...
        for (Pname n = d; n; n = nx) {
            nx = n->n_list;
            if (f->nrv && !re_decl
                && strcmp(f->nrv->string, n->string) == 0) { // found nrv in declaration
                n->string = new char[6];
    ...
    if (own_tbl) {
        ...
        for (m = memtbl->get_mem(i = 1); m; NEXT_NAME(memtbl, m, i)) {
            ...
            if (m->n_addr_taken == 0) {
                if (m->n_used) {
                    if (m->n_assigned_to == 0 && t->base != VEC && m->n_scope == FCT) {
                        Cdcl = m;
                        if (m->string[0] != '_' && m->string[1] != '_')
                            error('w', &m->where, "%n used but not set", m);
                    }
                } else {
                    if (m->n_assigned_to == 0 && (m->string[0] != '_' || m->string[1] != '_')
