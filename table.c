...
*/
{
    ...
    DB(if (Bdebug >= 1 && (Bdebarg == 0 || strcmp(Bdebarg, nx->string) == 0)) {
        ...
        error('d', "   %s t_name %s size %d free_slot %d", t_realbase == KTABLE ? "KTABLE" : "",
              this == gtbl ? "global table" : t_name ? t_name->string : "?", size, free_slot);
        if (Bdebug >= 2)
            display_expr(nx, "tbl::insert()");
    });
    ...
    if (nx->n_tbl_list || nx->n_table)
        error('i', "%n in two tables (n_tbl_list %d, n_table %d%n)", nx, nx->n_tbl_list, nx->n_table,
              nx->n_table ? nx->n_table->t_name : 0);
    /* use simple hashing with linear search for overflow */
...
void table::reinit() { // reuse table for stmt::simpl
...
Pname ktable::look(char *s, TOK k) {
    if (k_tiny) {
        for (Pname n = k_n; n; n = n->n_tbl_list)
            if (STRCMP(n->string, s) == 0 && n->n_key == k)
                return n;
        return 0;
    } else
        return k_t->look(s, k);
}
Pname ktable::find_cn(char *s) {
    if (k_id != CLASS)
