...
static class dcl_buf {
  ...
  public:
    ...
    void base(Pbase bb, bit const_flag = 0) {
        b = bb;
        cf = const_flag;
    }
    void front(TOK t) {
...
void dcl_buf::put() {
    ...
    if (MX <= li || MX <= ri)
        error('i', "T buffer overflow");

    if (n && n->n_sto && n->n_sto != REGISTER && n->n_sto != AUTO)
        puttok(n->n_sto);
        ...
        case COBJ: // "X a;" == "X a = {0};"
        {
                ...
                if (warning_opt) {
                    ...
                    if (obname == currname && tt == tn) {
                        if (cl->string[0] == '_' && cl->string[1] == '_')
                            error('w', &obname->where,
                                  "cannot generate zeroIr for%n ofT union; toI, compile with +a1 or "
                                  "insert non-union object as first member",
                                  obname);
                        else
                            error('w', &obname->where,
                                  "cannot generate zeroIr for%n ofT%t; toI, compile with +a1 or insert "
                                  "non-union object as first member",
                                  obname, obname->tp);
                    } else {
                        Pclass cx;
                        if (tn->base == COBJ && (cx = (Pclass) Pbase(tn)->b_name->tp)
                            && cx->string[0] == '_' && cx->string[1] == '_')
                            error('w', &obname->where,
                                  "cannot generate zeroIr for%n (union or aggregate with union as "
                                  "first element); toI, compile with +a1 or insert non-union object as "
                                  "first member",
                                  obname);
                        else
                            error('w', &obname->where,
                                  "cannot generate zeroIr for%n ofT%t (union or aggregate with union "
                                  "as first element); toI, compile with +a1 or insert non-union object "
                                  "as first member",
                                  obname, obname->tp);
                    }
                } // if warning_opt
            ...
            while (l) {
                if (l->base == NAME)
                    ...
                    return new expr(ILIST, zero, 0);
                l = cl->baselist;
            }
            int i = 1;
            Pname nn = cl->memtbl->get_mem(i);
            for (; nn; NEXT_NAME(cl->memtbl, nn, i)) {
                if (nn->base == TNAME || nn->tp == 0)
                    continue;
                Ptype tx = nn->tp->skiptypedefs();
                if (nn->base == NAME && nn->n_anon == 0 && tx->base != FCT && tx->base != OVERLOAD
                    && tx->base != CLASS && tx->base != ENUM && nn->n_stclass != STATIC
                    && nn->n_stclass != ENUM) {
...
*/
{
    ...
    for (Pname n = this; n; n = n->n_list) {
        ...
        if (t == 0)
            error('i', "N::dcl_print(%n)T missing", n);
        ...
        switch (t->base) {
            ...
            case FCT: {
                ...
                if (dtpt_opt && all_flag == 0 && curloc.file != first_file && f->f_inline == 0
                    && f->f_is_inline == 0 && !(n->n_sto == STATIC && !n->n_stclass) && none_flag == 0)
                    f->body = 0;

                if (t->is_templ_instance() && current_fct_instantiation != f) {
                    if (n->n_addr_taken == 1 && current_fct_instantiation == 0
                ...
                if (f->body && n->n_sto == EXTERN)
                    n->n_sto = 0;

                if (f->f_inline && (n->n_table != gtbl || strcmp(n->string, "main"))) {
                    if (debug_opt) {
                        if (f->defined & DEFINED && f->defined & SIMPLIFIED && f->f_inline != ITOR)
                            goto prnt_def;
                    ...
                    prnt_dcl:
                        ...
                        sm = 1; // no SM
                } else if ((f->defined & DEFINED) == 0 || (f->defined & SIMPLIFIED) == 0)
                    goto prnt_dcl;
                ...
                prnt_def:
                    ...
                    if (n->n_sto == STATIC && pdef_name == 0 && def_name == 0) {
                        pdef_name = n;
                        if (last_ll = n->where.line)
                            n->where.putline();
                        else
            ...
            tcx:
                ...
                if (tc && n->n_sto != EXTERN // extern const one;
                                             // const one = 1;
                                             // allocates storage
            ...
            default: {
                ...
                if (dtpt_opt && all_flag == 0 && n->n_scope != STATIC && none_flag == 0
                ...
                if (n->n_stclass == REGISTER) {
                    ...
                    if (cln) {
                        ...
                        if (cl->csu != CLASS && cl->baselist == 0 && cl->has_itor() == 0
                            && cl->virt_count == 0 && !n->n_addr_taken)
                            puttok(REGISTER);
                ...
                if (i) {
                    ...
                    if (tempt->base == COBJ) {
                        ...
                        if (cln) {
                            ...
                            if (n->n_table != gtbl && n->n_sto != EXTERN && n->n_evaluated == 0
                                && n_stclass == STATIC && ctor)
                                n->n_initializer = i = mk_zero_init(t, n, n);
                        }
                    }
                    if ((n->n_sto == 0 || n->n_val && n->n_evaluated == 0) && n_stclass == STATIC
                        && n->n_sto != STATIC && (n->n_table == gtbl || ispt_data(n))) {
                ...
                if (n->n_scope != ARG) {
                    if (i) {
                        ...
                        if (t != i->tp && i->base != ZERO && i->base != ILIST) {
...
char *local_sign(Ptype pt) { // get function signature for local class
    char buf[1024];
    char *bb = pt->signature(buf);
    int ll = bb - buf;
    if (1023 < ll)
        error('i', "local class N buffer overflow");
...
void enumdef::dcl_print(Pname cln) {
    ...
    for (Pname px, p = mem; p; p = px) {
        ...
        if (in_class == 0 || in_class->class_base != INSTANTIATED)
            delete p;
    }
    if (in_class == 0 || in_class->class_base != INSTANTIATED)
        mem = 0;
    else
        e_body = 3; // already printed
    puttok(RC);
    puttok(SM);
}

void name::print(bit fullprint) { // print just the name itself
    if (this == 0)
        error('i', "0->N::print()");
    if (string == 0) {
        if (emode)
            ...
            switch (n_oper) {
                ...
                case DTOR:
                    if (tbl && tbl->t_name) {
                        ...
                        if (string)
                            fprintf(out_file, "%s",
                                    n_template_arg_string ? n_template_arg_string : "destructor");
                        else
                ...
                case CTOR:
                    if (tbl && tbl->t_name)
                        ...
                        if (string)
                            fprintf(out_file, "%s",
                                    n_template_arg_string ? n_template_arg_string : "constructor");
                        else
                ...
                if (tbl = n_table) { // global or member
                    ...
                    if (tn = tbl->t_name) {
                        ...
                        if (asig) {
                            ...
                            cl = 0;
                        } else if (cl->string[0] == '_' && cl->string[1] == '_' && cl->string[2] == 'C'
                                   && n_stclass != STATIC) {
                            // error('d',"  inline%d vtbl_opt %d ansi_opt %d
                            // n_sto%k",Pfct(tp)->f_inline,vtbl_opt,ansi_opt,n_sto);
                            if (tp->base == FCT && (Pfct(tp)->f_is_inline && vtbl_opt)
            ...
            if (emode)
                ...
                if (Pclass(this)->nested_sig)
                    ...
                    fprintf(out_file, "struct %s *",
                            Pclass(this)->local_sig ? Pclass(this)->local_sig : Pclass(this)->string);
            } else
                    ...
                    for (Pname nn = f->argtype; n != nn; nn = nn->n_list) {
                        ...
                        if (nn->tp == n->tp || nn->tp->check(n->tp, 0) == 0) {
                            ...
                            while ((nnn = nnn->n_list) && x < 9) {
                                if (nnn->tp == n->tp || nnn->tp->check(n->tp, 0) == 0) {
                                    x++;
                                    n = nnn;
                                } else
                                    break;
...
*/
{
    ...
    while (t) {
        ...
        switch (t->base) {
            ...
            case TYPE:
                if ((Cast && TCast)
                    || Pbase(t)->b_name->lex_level) { // unravel type in case it contains vectors
                    Ptype tt = t;
                    bit ok = 0;
            ...
            dobase:
                if (emode) {
                    ...
                    switch (tt->base) {
                        case CLASS:
                            s = Pclass(tt)->string;
                            if (Ptclass(tt)->class_base == INSTANTIATED
...
void classdef::print_members() {
    ...
    for (Pname nn = memtbl->get_mem(i = 1); nn; NEXT_NAME(memtbl, nn, i)) {
        if (nn->base == TNAME)
            continue;
        if (nn->base == NAME && nn->n_anon == 0 && nn->tp->base != FCT && nn->tp->base != OVERLOAD
            && nn->tp->base != CLASS && nn->tp->base != ENUM && nn->n_stclass != STATIC) {
            Pexpr i = nn->n_initializer;
            nn->n_initializer = 0;
            DB(if (Pdebug >= 1) error('d', &nn->where, "  printingM%n base%k tbase%k n_key%k", nn,
                                      nn->base, nn->tp->base, nn->n_key););
            nn->dcl_print(0);
            nn->n_initializer = i;
        }
    }
...
int p2(Pname nn, Ptype t, Pclass cl, Pvirt vtab, char *s) {
    ...
    if (t->base == FCT) {
        ...
        if (nn->n_initializer || nn->n_sto == STATIC || f->f_inline || f->f_imeasure
            || f->f_virtual == 0)
            return 0;
        init = f->body != 0;
    } else
...
void really_really_print(Pclass cl, Pvirt vtab, char *s, char *ss) {
    ...
    if (nm = ptbl->look(vstr, 0)) {
        nm->n_key = HIDDEN;
        if (vstr != cstr && vstr != cs && vstr != cl->string)
            delete vstr;
    } else if (ptbl->look(vstr, HIDDEN) == 0)
        ptbl->insert(new name(vstr), HIDDEN);
    else if (vstr != cstr && vstr != cl->string && vstr != cs)
        delete vstr;
}
...
void classdef::dcl_print(Pname n) {
    ...
    DB(if (Pdebug >= 1)
           error('d', "%t::dcl_print(%n)   c_body %d defined%o", this, n, c_body, defined););
    ...
    for (Pname nn = memtbl->get_mem(i = 1); nn; NEXT_NAME(memtbl, nn, i)) {
        if (nn->base == TNAME)
            continue;
        if (nn->base == NAME && nn->n_anon == 0 && nn->tp->base == CLASS && Pclass(nn->tp)->c_body == 1)
            Pclass(nn->tp)->dcl_print(nn);
        else if (nn->base == TNAME && nn->tp->base != COBJ)
            nn->dcl_print(0);
        else if (nn->tp && nn->n_anon == 0 && nn->tp->base == ENUM && Penum(nn->tp)->e_body != 3)
...
char *make_local_name(Ptype tt, Pname fn) {
    char *buf;
    int tlen;
    char *tstring;
    switch (tt->base) {
