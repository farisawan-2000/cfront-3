...
*/
{
    ...
    if (f) {
        if (base == LDOUBLE || (t && t->base == LDOUBLE))
            return ldouble_type;
        if (base == DOUBLE || (t && t->base == DOUBLE))
            return double_type;
        return float_type;
    }
    if (l & u)
    ...
    if (l & !u)
        return long_type;
    if (u) {
        if (base == INT || (t && t->base == INT))
            return uint_type;
        if (SZ_SHORT == SZ_INT) // ANSIism
            if (base == SHORT || (t && t->base == SHORT))
                return uint_type;
        return int_type;
    }
...
int type::align() {
    ...
    switch (t->base) {
        case COBJ:
            return t->bname_type()->align();
        case VEC:
            return Pvec(t)->typ->align();
        case ANY:
            return 1;
        case CHAR:
            return AL_CHAR;
        ...
        case VOID:
            ...
            return AL_INT;
        default:
            error('i', "(%d,%k)->type::align", t, t->base);
            return 0;
    }
}
...
*/
{
    ...
    if (t == 0)
        error('i', "typ.tsizeof(t==0)");
    switch (t->base) {
        case COBJ:
            t = t->bname_type();
            if (t == 0)
                return 0; // ``fake'' generated classes: _Sdd
        ...
        case CLASS: {
            ...
            if ((cl->defined & (DEFINED | SIMPLIFIED)) == 0) {
                if (cl->class_base == CL_TEMPLATE || cl->class_base == UNINSTANTIATED)
                    error(
                        "YC%t not yet instantiated; please add an explicit instantiation (Tdef%t<Ts>)",
                        cl, cl);
                else
        ...
        default:
            return 0; // deref can be called for any type
